The `aws_s3` source now picks a working default for `force_path_style` when `use_fips_endpoint = true`, matching the recent `aws_s3` sink fix. Previously, `force_path_style` defaulted to `true`, which produced requests against the bare `s3-fips.<region>.amazonaws.com` hostname — but per [AWS][aws-fips], **all** S3 FIPS endpoints (commercial and GovCloud) only support virtual-hosted-style addressing. The result was every FIPS request failing with a dispatch error.
