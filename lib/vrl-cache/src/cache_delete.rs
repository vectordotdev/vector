...
impl Function for CacheDelete {
    ...
    fn examples(&self) -> &'static [Example] {
        &[Example {
            ...
            source: r#"cache_detel!("test_cache", "test_key")"#,
            result: Ok(""),
        }]
    }
...
#[derive(Debug, Clone)]
pub struct CacheDeleteFn {
    cache: String,
    key: Box<dyn Expression>,
    registry: VrlCacheRegistry,
}

impl FunctionExpression for CacheDeleteFn {
    fn resolve(&self, ctx: &mut Context) -> Resolved {
        ...
        Ok(self
            .registry
            .writer()
            .delete_val(&self.cache, &key)
            .ok_or_else(|| format!("key not found in cache: {key}"))?)
    }

    fn type_def(&self, _: &TypeState) -> TypeDef {
        TypeDef::any().impure().fallible()
    }
}
...
mod tests {
    ...
    fn delete_val() {
        ...
        assert_eq!(None, registry.as_readonly().get_val("test", "test_key"));
    }

    #[test]
    fn delete_val_key_not_found() {
        let registry = get_cache_registry();
        let func = CacheDeleteFn {
            cache: "test".to_string(),
            key: expr!("test_key"),
