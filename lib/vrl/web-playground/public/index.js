...
let ERROR_INVALID_JSONL_EVENT_MSG = `Error attempting to parse the following string into valid JSON\n
String: {{str}}
...
export class VrlWebPlayground {
    constructor() {
        let temp = init().then(() => {
            ...
            require(["vs/editor/editor.main"], () => {
                this.monaco = monaco;
                this.programEditor = this.createDefaultEditor("container-program", PROGRAM_EDITOR_DEFAULT_VALUE, "coffeescript");
                this.eventEditor = this.createDefaultEditor("container-event", EVENT_EDITOR_DEFAULT_VALUE, "json");
                this.outputEditor = this.createDefaultEditor("container-output", OUTPUT_EDITOR_DEFAULT_VALUE, "json");
    ...
    createDefaultEditor(elementId, value, language) {
    ...
    getState() {
        ...
        return {
            program: this.programEditor.getValue(),
            event: JSON.parse(this.eventEditor.getValue()),
            is_jsonl: false,
        };
    }
    ...
    tryJsonParse(str) {
        try {
            ...
            this.outputEditor.setValue(ERROR_INVALID_JSONL_EVENT_MSG.replace("{{str}}", str));
        }
    }
