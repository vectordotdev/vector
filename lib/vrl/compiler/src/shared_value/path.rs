...
impl SharedValue {
    ...
    pub fn at_path(self, path: &LookupBuf) -> SharedValue {
        for segment in path.as_segments().iter().rev() {
            match segment {
                SegmentBuf::Field(FieldBuf { name, .. }) => {
                    let mut map = BTreeMap::default();
                    map.insert(name.as_str().to_owned(), self.clone());
                    self.replace(Value::Object(map));
                }
                SegmentBuf::Coalesce(fields) => {
                    let field = fields.last().unwrap();
                    let mut map = BTreeMap::default();
                    map.insert(field.as_str().to_owned(), self.clone());
                    self.replace(Value::Object(map));
                }
                SegmentBuf::Index(index) => {
                    ...
                    if *index > 0 {
                        array.resize(*index as usize, SharedValue::from(Value::Null));
                    }

                    array.push(self.clone());
                    self.replace(Value::Array(array));
                }
            }
        }
...
mod tests {
    ...
    fn test_object() {
        ...
        let value = shared_value!(12);

        let object = shared_value!({ "foo": { "bar": { "baz": 12 } } });
    ...
    fn test_array() {
        ...
        let object = shared_value!([null, null, 12]);
    ...
    fn test_complex() {
        ...
        let value = shared_value!({ "bar": [12] });

        let object = shared_value!([null, null, { "foo": { "baz": [null, { "bar": [12] }] } } ]);
