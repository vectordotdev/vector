    ...
    loop {
        match (path_iter.next(), value) {
            ...
            (Some(BorrowedSegment::CoalesceField(key)), Value::Object(map)) => {
                ...
                value = map.get_mut_value(matched_key.as_ref()).expect("this was already checked to exist");
            }
...
mod test {
    ...
    fn test_coalesce() {
        ...
        assert_eq!(
            Value::from(json!({"b": {"x": 5}})).get_mut("(a|b).x").cloned(),
            Some(Value::from(5))
        );
        assert_eq!(
            Value::from(json!({"b": {"x": 5}})).get_mut("(a|b).(y|x)").cloned(),
            Some(Value::from(5))
        );
        assert_eq!(
        ...
        assert_eq!(
            Value::from(json!({})).get_mut("(a|b|c)").cloned(),
            None
        );
    }
}
