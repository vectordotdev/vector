...
/// Produces a `LazyLock<Value>` that merges extra fields into a cloned base tag set.
///
/// ```ignore
/// pub static MY_TAGS: LazyLock<Value> = merge_lazy!(COMPONENT_TAGS, {
///     "bucket": {"description": "The S3 bucket.", "required": true}
/// });
/// ```
macro_rules! merge_lazy {
    ($base:expr, { $($tt:tt)* }) => {
        LazyLock::new(|| merge(&*$base, json!({ $($tt)* })))
    };
}
...
pub static COMPONENT_TAGS: LazyLock<Value> = merge_lazy!(INTERNAL_METRICS_TAGS, {
    "component_kind": {
...
pub static COMPONENT_TAGS_OUTPUT: LazyLock<Value> = merge_lazy!(COMPONENT_TAGS, {
    "output": {"description": "The specific output of the component.", "required": false}
});

pub static COMPONENT_TAGS_GRPC_METHOD_SERVICE: LazyLock<Value> = merge_lazy!(COMPONENT_TAGS, {
...
pub static COMPONENT_TAGS_GRPC_ALL: LazyLock<Value> = merge_lazy!(COMPONENT_TAGS_GRPC_METHOD_SERVICE, {
    "grpc_status": {"description": "The human-readable gRPC status code.", "required": true}
});

pub static COMPONENT_TAGS_HTTP_METHOD: LazyLock<Value> = merge_lazy!(COMPONENT_TAGS, {
    "method": {"description": "The HTTP method of the request.", "required": false}
});

pub static COMPONENT_TAGS_HTTP_STATUS: LazyLock<Value> = merge_lazy!(COMPONENT_TAGS, {
    "status": {"description": "The HTTP status code of the request.", "required": false}
});

pub static COMPONENT_TAGS_HTTP_METHOD_PATH: LazyLock<Value> = merge_lazy!(COMPONENT_TAGS_HTTP_METHOD, {
    "path": {"description": "The path that produced the error.", "required": true}
});

pub static COMPONENT_TAGS_HTTP_ALL: LazyLock<Value> = merge_lazy!(COMPONENT_TAGS_HTTP_METHOD_PATH, {
    "status": {"description": "The HTTP status code of the request.", "required": false}
});

pub static COMPONENT_TAGS_ERROR_TYPE_STAGE: LazyLock<Value> = merge_lazy!(COMPONENT_TAGS, {
...
pub static INTERNAL_METRICS_TAGS_FILE: LazyLock<Value> = merge_lazy!(INTERNAL_METRICS_TAGS, {
    "file": {"description": "The file that produced the error.", "required": false}
});

pub static INTERNAL_METRICS_TAGS_REASON: LazyLock<Value> = merge_lazy!(INTERNAL_METRICS_TAGS, {
...
pub static COMPONENT_RECEIVED_EVENTS_TOTAL_TAGS: LazyLock<Value> = merge_lazy!(COMPONENT_TAGS, {
...
pub static S3_OBJECT_PROCESSING_TAGS: LazyLock<Value> = merge_lazy!(COMPONENT_TAGS, {
