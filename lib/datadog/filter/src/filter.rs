...
/// function. Each method returns a heap-allocated `Matcher<T>` (typically a closure) containing
/// logic to determine whether the value matches the filter.
pub trait Filter<'a, T: std::fmt::Debug + Send + Sync + Clone + 'static>:
    DynClone + Send + Sync
{
    /// Determine whether a field value exists.
    fn exists(&'a self, field: Field) -> Box<dyn Matcher<T>>;

    /// Determine whether a field value equals `to_match`.
    fn equals(&'a self, field: Field, to_match: String) -> Box<dyn Matcher<T>>;
    ...
    fn range(
        ...
        match (&lower, &upper) {
            ...
            _ => {
                ...
                Run::boxed(move |value: &T| lower_func.run(value) && upper_func.run(value))
            }
        }
    }
}

clone_trait_object!(<T>Filter<'_, T>);

fn any<T>(queries: Vec<Box<dyn Matcher<T>>>) -> Box<dyn Matcher<T>>
where
...
fn all<T>(queries: Vec<Box<dyn Matcher<T>>>) -> Box<dyn Matcher<T>>
where
    T: std::fmt::Debug + Send + Sync + Clone + 'static,
{
...
pub fn build_filter<'a, T, F>(node: &QueryNode, f: &'a mut F) -> Box<dyn Matcher<T>>
where
    T: std::fmt::Debug + Send + Sync + Clone + 'static,
    F: Fielder + Filter<'a, T>,
{
    match node {
