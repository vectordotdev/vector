...
impl vrl::Target for VrlTarget {
    fn target_insert(&mut self, path: &LookupBuf, value: vrl::Value) -> Result<(), String> {
        match self {
    ...
    fn target_get(&self, path: &LookupBuf) -> std::result::Result<Option<vrl::Value>, String> {
        match self {
    ...
    fn get_metadata(&self, key: &str) -> Result<Option<vrl::Value>, String> {
        ...
        match key {
            "datadog_api_key" => Ok(metadata
                .datadog_api_key()
                .as_ref()
                .map(|api_key| vrl::Value::from(api_key.to_string()))),
            "splunk_hec_token" => Ok(metadata
                .splunk_hec_token()
                .as_ref()
                .map(|token| vrl::Value::from(token.to_string()))),
...
mod test {
    ...
    fn log_get() {
        ...
        for (value, segments, expect) in cases {
            ...
            assert_eq!(vrl::Target::target_get(&target, &path), expect);
        }
    }
    ...
    fn log_insert() {
        ...
        for (object, segments, value, expect, result) in cases {
            ...
            assert_eq!(
                vrl::Target::target_insert(&mut target, &path, value.clone()),
                result
            );
            assert_eq!(vrl::Target::target_get(&target, &path), Ok(Some(value)));
    ...
    fn log_remove() {
        ...
        for (object, segments, compact, expect) in cases {
            let mut target = VrlTarget::new(Event::Log(LogEvent::from(object)));
            let path = LookupBuf::from_segments(segments);
            let removed = vrl::Target::target_get(&target, &path).unwrap();

            assert_eq!(
                vrl::Target::target_remove(&mut target, &path, compact),
                Ok(removed)
            );
            assert_eq!(
                vrl::Target::target_get(&target, &LookupBuf::root()),
                Ok(expect)
            );
        }
    }
    ...
    fn log_into_events() {
        ...
        let cases = vec![
            ...
            (vrl::Value::from(1), vec![btreemap! {"message" => 1}]),
            (vrl::Value::from("2"), vec![btreemap! {"message" => "2"}]),
            (vrl::Value::from(true), vec![btreemap! {"message" => true}]),
            (
                vrl::Value::from(vec![
                    vrl::Value::from(1),
                    vrl::Value::from("2"),
        ...
        for (value, expect) in cases {
            ...
            vrl::Target::target_insert(&mut target, &LookupBuf::root(), value).unwrap();
