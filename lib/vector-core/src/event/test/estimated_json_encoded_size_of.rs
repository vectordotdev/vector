...
fn log_operation_maintains_size() {
    ...
    fn inner(actions: Vec<Action>, mut log_event: LogEvent) -> TestResult {
        let mut current_size = log_event.estimated_json_encoded_size_of();
        ...
        while let Some(action) = iter.next() {
            ...
            match action {
                Action::InsertFlat { key, value } => {
                    let new_value_sz = value.estimated_json_encoded_size_of();
                    ...
                    if !log_event.contains(key.as_str()) {
                        ...
                        current_size +=
                            key.estimated_json_encoded_size_of() + COLON_SIZE + COMMA_SIZE;
                    }
                ...
                Action::SizeOf => {
                    ...
                    if current_size != log_event.estimated_json_encoded_size_of() {
                        eprintln!("----------------------------");
                        eprintln!(
                            "start event ({}): {}",
                            start_event.estimated_json_encoded_size_of(),
                            start_event.value()
                        );
                        eprintln!(
                            "final event ({}): {}",
                        ...
                        assert_eq!(current_size, log_event.estimated_json_encoded_size_of(),);
                        eprintln!("----------------------------");
                    }
                ...
                Action::Remove { key } => {
                    ...
                    current_size -= key.estimated_json_encoded_size_of();
                }
            }
        }
