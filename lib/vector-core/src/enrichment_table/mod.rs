...
/// The Enrichment Tables manages the collection of EnrichmentTables loaded into Vector.
...
impl vrl_core::EnrichmentTables for EnrichmentTables {
    ...
    fn find_table_row<'a>(
        ...
        let tables = self.tables.load();
        match **tables {
            Some(ref tables) => match tables.get(table) {
                None => Err(format!("table {} not loaded", table)),
                Some(table) => Ok(table.find_table_row(criteria).map(|t| t.clone())),
            },
            None => Err("finish_load not called".to_string()),
        }
    }
...
mod tests {
    use super::*;
    use enrichment_table::EnrichmentTable;
    use shared::btreemap;
    use std::collections::BTreeMap;
    use std::sync::Arc;
    ...
    impl DummyEnrichmentTable {
        ...
        fn new_with_index(index: Arc<Mutex<Vec<Vec<String>>>>) -> Self {
            Self {
                data: vec!["result".to_string()],
                indexes: index.clone(),
            }
