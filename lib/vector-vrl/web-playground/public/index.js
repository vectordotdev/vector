...
export class VrlWebPlayground {
    ...
    _renderRunResult(runResult) {
        const elapsedEl = document.getElementById("elapsed-time");
        if (elapsedEl && runResult?.elapsed_time != null) {
            const ms = runResult.elapsed_time.toFixed(2)
            elapsedEl.textContent = runResult?.elapsed_time != null ? `Duration: ${ms} milliseconds` : "";
        }

        if (runResult?.target_value != null) {
            const isJson = typeof runResult.target_value === "object" && runResult.target_value !== null;
        ...
        if (runResult?.msg != null) {
            this.outputEditor.setValue(runResult?.msg);
            return;
        }

        // Ultimate fallback so the output window never stays empty
        this.outputEditor.setValue("Error - VRL did not return a result.");
    }
    ...
    handleRunCode(input) {
        ...
        this._renderRunResult(runResult);
        return runResult;
    }
    ...
    handleRunCodeJsonl() {
        ...
        const outputs = results.map(run_result => {
            if (run_result?.target_value != null) {
                return JSON.stringify(run_result.target_value, null, "\t");
            } else if (run_result?.msg != null) {
                return run_result.msg;
            }
            return "Error - VRL did not return a result.";
        });
