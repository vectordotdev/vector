...
fn benchmark_insert(c: &mut Criterion) {
    let mut group = c.benchmark_group("hash_map::insert");
...
fn benchmark_get(c: &mut Criterion) {
    let mut group = c.benchmark_group("hash_map::get");
    for param in &PARAMETERS {
        group.throughput(Throughput::Bytes(
            (param.loops * mem::size_of::<u64>() * 2) as u64,
        ));
...
fn benchmark_contains_key(c: &mut Criterion) {
    let mut group = c.benchmark_group("hash_map::contains_key");
    for param in &PARAMETERS {
        group.throughput(Throughput::Bytes(
            (param.loops * mem::size_of::<u64>() * 2) as u64,
        ));
...
fn benchmark_clone(c: &mut Criterion) {
    let mut group = c.benchmark_group("hash_map::clone");
    for param in &PARAMETERS {
        group.throughput(Throughput::Bytes(
            (param.loops * mem::size_of::<u64>() * 2) as u64,
        ));
