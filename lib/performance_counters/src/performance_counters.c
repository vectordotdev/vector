// Based on
// https://gist.githubusercontent.com/ibireme/173517c208c7dc333ba962c1f0d67d12/raw/c04408539d3444e457a5b400fa2ffb18aa2ef8be/kpc_demo.c.
...
static const event_alias profile_events[] = {
    ...
    //      12 "BRANCH_MISPREDICT",            // Apple A7-A14
    //      "BR_MISP_RETIRED.ALL_BRANCHES", // Intel Core 2th-10th
    //      "BR_INST_RETIRED.MISPRED",      // Intel Yonah, Merom
    //  }},
    {"load_store_instructions",
     {
         "INST_LDST", // Apple A7-A15
     }},
    {"l1-data-load-cache-misses",
     {
         "L1D_CACHE_MISS_LD_NONSPEC", // Apple A7-A15
     }},
    {"l1-data-store-cache-misses",
     {
         "L1D_CACHE_MISS_ST_NONSPEC", // Apple A7-A15
     }},
};
...
static kpep_config *CFG = NULL;
static const usize EV_COUNT =
    sizeof(profile_events) / sizeof(profile_events[0]);
static u32 CLASSES = 0;
static usize reg_count = 0;
// prepare buffer and config
static kpc_config_t REGS[KPC_MAX_COUNTERS] = {0};
...
int init_counting() {
  ...
  for (usize i = 0; i < EV_COUNT; i++) {
    ...
    if ((ret = kpep_config_add_event(CFG, &ev, 0, NULL))) {
      printf("Failed to add event %s: %d (%s).\n", alias->alias, ret,
             kpep_config_error_desc(ret));
      return 1;
    }
  }
...
int get_counters(counters *counters) {
  int ret = 0;

  // get counters before
  if ((ret =
           kpc_get_thread_counters(0, KPC_MAX_COUNTERS, counters->counters))) {
    printf("Failed get thread counters: %d.\n", ret);
    return 1;
  }
...
void print_counters(counters *counters_0, counters *counters_1) {
  // result
  printf("counters value:\n");
  for (usize i = 0; i < EV_COUNT; i++) {
    const event_alias *alias = profile_events + i;
    usize idx = COUNTER_MAP[i];
    u64 val = counters_1->counters[idx] - counters_0->counters[idx];
    printf("%14s: %llu\n", alias->alias, val);
  }
}

typedef struct named_counters {
  u64 cycles;
  u64 load_store_instructions;
  u64 l1_data_load_cache_misses;
  u64 l1_data_store_cache_misses;
} named_counters;

named_counters get_named_counters(counters *counters_0, counters *counters_1) {
  named_counters result = {
      .cycles = counters_1->counters[COUNTER_MAP[0]] -
                counters_0->counters[COUNTER_MAP[0]],
      .load_store_instructions = counters_1->counters[COUNTER_MAP[1]] -
                                 counters_0->counters[COUNTER_MAP[1]],
      .l1_data_load_cache_misses = counters_1->counters[COUNTER_MAP[2]] -
                                   counters_0->counters[COUNTER_MAP[2]],
      .l1_data_store_cache_misses = counters_1->counters[COUNTER_MAP[3]] -
                                    counters_0->counters[COUNTER_MAP[3]],
  };
  return result;
}

counters init_counters() {
  counters init = {.counters = {0}};
  return init;
}
