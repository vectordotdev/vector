...
async fn reader_throws_error_when_record_is_undecodable_via_metadata() {
    ...
    with_temp_dir(|dir| {
        ...
        async move {
            ...
            let ReaderError::Incompatible { reason: second_read_error_reason } = second_read_result.unwrap_err()
            else { panic!("error should be ReadError::Incompatible") };
            ...
            let ReaderError::Incompatible { reason: third_read_error_reason } = third_read_result.unwrap_err()
            else { panic!("error should be ReadError::Incompatible") };
