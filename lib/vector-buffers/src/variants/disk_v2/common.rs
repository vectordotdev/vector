...
impl DiskBufferConfigBuilder {
    ...
    pub fn build(self) -> DiskBufferConfig {
        // TODO: Make config building fallible so we can validate our values, as part of satisfying
        // some of the established lemmas, namely lemma 6.
        //
        // We can utilize `get_on_disk_record_archive_len` or something to assert this, although
        // it'd be nice to have a slightly higher level way, like a dedicated type we do all
        // encoding/framing with that encapsulates the entire process of going from T to a fully
        // length-delimited and archived record on disk.
