    ...
    match mode {
        WhenFull::Overflow => {
            ...
            let (overflow_sender, overflow_receiver) =
                TopologyBuilder::standalone_memory(capacity, overflow_mode).await;
            let (mut base_sender, mut base_receiver) =
                TopologyBuilder::standalone_memory(capacity, WhenFull::Overflow).await;
        ...
        m => TopologyBuilder::standalone_memory(capacity, m).await,
    }
}
