...
fn generate_series_intake(payloads: &[MetricPayload]) -> SeriesIntake {
    ...
    payloads.iter().for_each(|payload| {
        payload.series.iter().for_each(|serie| {
            ...
            if !serie.metric.starts_with("foo_metric") || serie.metric == RESOURCE_METRIC_NAME {
                return;
            }
...
fn assert_v2_resource(series: &[MetricPayload]) {
    let metric_names: Vec<_> = series
        .iter()
        .flat_map(|payload| &payload.series)
    ...
    let series = series
        ...
        .find(|series| series.metric == RESOURCE_METRIC_NAME)
        .unwrap_or_else(|| {
            panic!("resource metric should be present in the V2 payload; got {metric_names:?}")
        });
    ...
    assert!(
        series
            .resources
            .iter()
            .any(|resource| resource.r#type == RESOURCE_TYPE && resource.name == RESOURCE_NAME),
        "V2 payload should contain the database instance resource"
    );
    assert!(
        !series
            .tags
