...
// Re-exported at `pub(in crate::datadog::metrics)` so sibling modules (e.g. `v3`) can build a `MetricPayload` from
// their own decoded shape and reuse `generate_series_intake`/`common_series_assertions`/
// `compare_intakes` below, rather than reimplementing series comparison from scratch.
pub(in crate::datadog::metrics) use ddmetric_proto::{
    MetricPayload,
...
pub(in crate::datadog::metrics) use self::ddmetric_proto::metric_payload::MetricPoint;
use self::ddmetric_proto::metric_payload::Resource;
use super::*;
...
pub(in crate::datadog::metrics) struct SeriesContext {
    metric_name: String,
    tags: Vec<String>,
    r#type: i32,
}
...
#[derive(Clone, Debug, Eq, Hash, PartialEq, PartialOrd, Ord)]
pub(in crate::datadog::metrics) struct TimeBucket(i64, i64);
...
pub(in crate::datadog::metrics) type SeriesIntake = BTreeMap<SeriesContext, TimeSeriesData>;

// massages the raw payloads into our intake structure
pub(in crate::datadog::metrics) fn generate_series_intake(
...
// of the pipeline
pub(in crate::datadog::metrics) fn common_series_assertions(series: &SeriesIntake) {
...
pub(in crate::datadog::metrics) fn compare_intakes(
    agent_intake: &SeriesIntake,
    vector_intake: &SeriesIntake,
) {
