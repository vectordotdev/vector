...
use antithesis_harness::{OracleClient, VectorClient};
...
async fn main() {
    ...
    let vector = VectorClient::new(http, args.source_url);

    let Some(id) = oracle.claim().await else {
    ...
    for _ in 0..MAX_ATTEMPTS {
        ...
        // retry the same id.
        if vector.post_event(id, time::Duration::from_secs(5)).await {
            ...
            if oracle.report_acked(id).await {
