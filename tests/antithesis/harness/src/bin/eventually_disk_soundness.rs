...
fn parse_buffer_metrics(body: &str) -> Option<BufferMetrics> {
    // The maximum-size gauge is emitted when the buffer is created. Current
    // occupancy gauges are registered lazily, so their absence means the
    // recovered buffer has not received or sent any events yet.
    disk_metric_sum(body, "vector_buffer_max_size_bytes")?;

    Some(BufferMetrics {
        occupancy_events: disk_metric_sum(body, "vector_buffer_size_events").unwrap_or_default(),
        occupancy_bytes: disk_metric_sum(body, "vector_buffer_size_bytes").unwrap_or_default(),
...
mod tests {
    ...
    #[test]
    fn max_size_metric_marks_an_empty_buffer_ready() {
        let body = r#"
...
"#;
        ...
        assert_eq!(metrics.received_events, 0.0);
        assert_eq!(metrics.sent_events, 0.0);
...
"#;
        ...
        assert_eq!(metrics.occupancy_events, 1.0);
        assert_eq!(metrics.occupancy_bytes, 524_496.0);
        assert_eq!(metrics.received_events, 12.0);
        assert_eq!(metrics.sent_events, 11.0);
        assert_eq!(metrics.discarded_events, 2.0);
    }
