...
//!
//! Each Antithesis invocation finds the highest-numbered data file, which is
//! the writer's active segment in this bounded scenario, and flips two bytes of
//! the final complete record's checksum without changing the file length. A
//! marker limits the mutation to once per segment so repeated command scheduling
//! cannot restore the original checksum or repeatedly corrupt the same file.
...
fn active_data_file(buffer_dir: &Path) -> io::Result<Option<DataFile>> {
    ...
    for entry in fs::read_dir(buffer_dir)? {
        ...
        if active.as_ref().is_none_or(|file: &DataFile| id > file.id) {
            active = Some(DataFile { id, path });
        }
    }
    Ok(active)
}

fn claim_marker(state_dir: &Path, data_file_id: u16) -> io::Result<Option<(PathBuf, File)>> {
    fs::create_dir_all(state_dir)?;
    let marker_path = state_dir.join(format!("buffer-data-{data_file_id}.claimed"));
...
fn corrupt_active_data_file_with(
    ...
    let Some((marker_path, marker)) = claim_marker(state_dir, candidate.id)? else {
        return Ok(None);
    };
...
"#;
    ...
    #[test]
    fn highest_numbered_data_file_is_active() {
        let temp = tempfile::tempdir().unwrap();
        fs::write(temp.path().join("buffer-data-3.dat"), [0; 8]).unwrap();
        fs::write(temp.path().join("buffer-data-5.dat"), [0; 8]).unwrap();
        fs::write(temp.path().join("buffer-data-4.dat"), [0; 8]).unwrap();
        fs::write(temp.path().join("buffer-data-6.dat"), b"").unwrap();

        let active = active_data_file(temp.path()).unwrap().unwrap();
