...
### Recovery: `roll_to_next_data_file` (reader.rs:705-753)

Called from `BufferReader::next` at reader.rs:1047 when `e.is_bad_read()` is true:
...
`roll_to_next_data_file` (reader.rs:705-753):
...
- Adds a data-file deletion marker to `self.data_file_acks` (reader.rs:740-746), which eventually drives `delete_completed_data_file` once all records up to that point are acknowledged.
- Calls `self.reset()` (reader.rs:749), which sets `self.reader = None`, zeroes `bytes_read`, and clears `data_file_start_record_id`.
- Calls `self.ledger.increment_unacked_reader_file_id()` (reader.rs:750), advancing the reader to the next data file.
...
Also called at reader.rs:1086 in the `Ok(None)` path (end-of-file when `reader_file_id != writer_file_id`):
...
And at reader.rs:923 in `seek_to_next_record` (during initialization bad-read handling), though that path uses a slightly different flow (checking `reader_file_id > writer_file_id` at reader.rs:943 to avoid deadlock).

### What `roll_to_next_data_file` Does NOT Do

It does not immediately delete the corrupted file. Deletion is deferred to `delete_completed_data_file` via the `data_file_acks` queue, which requires the record-level acknowledgements to drain first. A corrupted file that was partially read (some records read and acked before the corruption was hit) will be deleted only after those earlier records are acked. A file that was corrupted on the very first read (`data_file_start_record_id` is `None`) uses `last_reader_record_id` as a zero-length marker (reader.rs:709-716).
...
For example: data file has records [A, B, CORRUPT, D, E]. Reader reads A, B, hits CORRUPT, calls `roll_to_next_data_file`. Records D and E are abandoned without being read, acknowledged, or counted. They are charged to `decrement_total_buffer_size` via `delete_completed_data_file`'s `size_delta` calculation (reader.rs:535, with the underflow assert at reader.rs:529), but they are never delivered.

This is intentional per the code comment at reader.rs:1026-1035:
...
- **Truncation of the data file mid-record**: With `is_finalized=true`, `try_next_record` detects insufficient bytes and returns `PartialWrite` (reader.rs:264, 328). `is_bad_read()` is true.
...
## SUT-Side Instrumentation Suggestions (SDK present; these asserts not yet committed)
...
(ledger.rs:271/313, reader.rs:529; see existing-assertions.md). The corruption
reachability assertions below are not among those three and remain still-to-add.

**Primary assertion** — in `BufferReader::next`, in the `is_bad_read()` branch just before `roll_to_next_data_file()` is called (reader.rs:1046-1047):
...
The `as_error_code()` method (reader.rs:141-150) already distinguishes the three bad-read variants (`"checksum_mismatch"`, `"deser_failed"`, `"partial_write"`). Antithesis can break down by error code to confirm all fault types are being detected.

**Secondary assertion** — in `roll_to_next_data_file`, to confirm the rolling logic itself completes (reader.rs:749-750):
