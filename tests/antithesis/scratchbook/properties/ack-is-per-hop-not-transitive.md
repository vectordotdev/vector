...
- The source finalizer is consumed at encode time into head's disk buffer (see
  `ack-does-not-imply-durability`), so head's 200 fires on **local durable-write**,
  independent of tail.
- The disk **reader mints a fresh `BatchNotifier`** for each record as it reads it
  out toward the downstream hop (`reader.rs:1128-1129`); that fresh notifier's acks
  drive only head's own record/file deletion — they are a *separate* ack chain from
  head→tail, which is itself separate from tail→collector.
...
- Client POSTs to head, gets 200 (head ack recorded). **Partition head↔tail** so
  head's buffer cannot drain, keep the head-acked id in head's buffer, then **drop
  head's buffer** (reload #24948 or crash-before-fsync). The id was acked at the
...
chain (`reader.rs:1128-1129`); there is no transitive end-to-end ack. The tail
collector is therefore the *only* truth for end-to-end delivery — exactly how the
harness treats it.

## Evidence trail

2026-06-02 Vector-durability chorus agent, current tree: `reader.rs:1128-1129`
