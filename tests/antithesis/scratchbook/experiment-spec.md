...
This is an **exhibition** experiment, not a certification. The claim under test,
as people state it: *"turn on e2e acks + a disk buffer and Vector is durable — if
it says ack, it comes out the other side; chain such durable nodes and you lose
nothing end to end."* The goal is to **build that exact advertised chain and force
...
- **corruption / torn tail** of acked-unflushed data, skipped on reopen — the code
  ...
  (`reader.rs:111-115`).
- **#21683** `total_buffer_size` underflow wedges the writer (a separate failure:
...
`assert_always(missing == 0)` gives the verdict + counterexample, but to drive the
search engine *toward* a loss timeline we also emit, on the confirmed-loss branch
at quiescence, `assert_unreachable("an end-to-end-acked event was permanently
lost")`. Antithesis actively searches for a fault schedule that reaches it;
reaching it IS the demonstration. The obligation model stays producer-witnessed
(one id per invocation) — the ack-window seam is irrelevant to exhibition (a
batch/systematic loss is caught in nearly every timeline; the seam can only hide a
single event, never the bug). SUT-side durable-acceptance reporting would be
needed only to *certify* zero loss, which is not this experiment's goal.
...
(Original bug-hunt framing retained below; the #21683 writer-deadlock remains a
target, caught by the probe + SUT-side underflow assert.)
...
- `N ∈ delivered` ≡ `N` actually emerged at the terminal collector.

## The oracle — plain integer-id sets, no watermark, no order

Order carries **no** information end-to-end: ingest order ≠ id-assignment order
(concurrent POSTs race for the single writer `Mutex`, `prelude.rs:193/286`), and
egress reorders + duplicates (sink `FuturesUnordered`, adaptive concurrency, retries,
`vector-stream/src/driver.rs:87`). Transport is **at-least-once**: duplicates are
legal, not bugs. Therefore the only sound oracle is **set membership**.
...
**Definition of loss:** Vector accepted durable responsibility for an event and it
never emerged — exactly `∃N. N ∈ acked ∧ N ∉ delivered`.

**Assertion 2** is `acked ⊆ delivered`, which is logically identical to:

```
∀N. (N ∈ acked → N ∈ delivered)  ≡  ¬∃N.(N ∈ acked ∧ N ∉ delivered)  ≡  "no lost event exists"
```

So the subset **failing IS the definition of a lost event existing**, and the
failing element names the dropped id. We evaluate it **at quiescence** (producers
stopped, in-flight stable, no active fault): before quiescence an acked-not-yet-
delivered id is legal in-flight; at rest, undelivered can only mean lost.

It is **subset, not equality**: `delivered` can legitimately be a strict superset
of `acked` (an event whose 200 was issued but whose ack-back was lost still gets
...
- **Collector is the oracle, not the SUT — exempt it from state-wiping faults**
  (do not include it in node-termination/hang) **and persist its sets** to the
  shared volume. A wiped collector is both a false miss and a false red.
...
real RNG seeded from Antithesis entropy (`random_range`/`choose`, never
`get_random() % N`), sizes kept **below `max_record_size`** and clustered at the
buffer boundaries (`0, 1, k·data_file_size ± 1`).
...
1. **The events gauge does NOT detect #21683 (critical).** The bug underflows the
   *bytes* counter `total_buffer_size` (the `fetch_sub` at `ledger.rs:319`) and wedges the writer; the
   ...
   deadlock detector.** The deadlock is caught two ways instead:
...
2. **Ungate loss + integrity (critical).** The old oracle gated both behind
   ...
   (immediate `assert_always` on an un-issued id). Loss (`acked ⊆ delivered`,
   i.e. `missing_count == 0`) is asserted **unconditionally at quiescence**, never
   gated on a buffer gauge.
