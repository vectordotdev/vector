...
fn basic_config() -> Config {
    ...
    config.add_source("in1", source().1, vec![].into(), vec![].into());
...
async fn topology_shutdown_while_active() {
    ...
    let mut config = Config::builder();
    config.add_source("in1", source1, vec![].into(), vec![].into());
...
async fn topology_multiple_sources() {
    ...
    let mut config = Config::builder();
    config.add_source("in1", source1, vec![].into(), vec![].into());
    config.add_source("in2", source2, vec![].into(), vec![].into());
...
async fn topology_healthcheck_run_for_changes_on_reload() {
    ...
    let (_ch0, src) = source();
    config.add_source("in1", src, vec![].into(), vec![].into());
    config.add_sink("out1", &["in1"], sink(10).1);
