...
2. Build Vector using Docker
  ...
  The command above builds a Docker image and then starts a container
  with it. Note that the container ought to be privileged because it registers
  `binfmt` handlers which allow to run tests for non-native architectures inside them.
