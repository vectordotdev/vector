...
description: "The Vector `tcp` sink streams `log` events to a TCP connection."
event_types: ["log"]
...
The Vector `tcp` sink [streams](#streaming) [`log`][docs.data-model.log] events to a TCP connection.
