...
description: "The `aws_kinesis_firehose` sink batches `log` events to AWS Kinesis Data Firehose via the `PutRecordBatch` API endpoint."
event_types: ["log"]
...
status: "beta"
title: "aws_kinesis_firehose sink"
unsupported_operating_systems: []
---

The `aws_kinesis_firehose` sink [batches](#buffers--batches) [`log`][docs.data-model.log] events to [AWS Kinesis Data Firehose][urls.aws_kinesis_data_firehose] via the [`PutRecordBatch` API endpoint](https://docs.aws.amazon.com/firehose/latest/APIReference/API_PutRecordBatch.html).
...
[sinks.my_sink_id]
  # REQUIRED - General
  type = "aws_kinesis_firehose" # example, must be: "aws_kinesis_firehose"
...
The [stream name][urls.aws_cw_logs_stream_name] of the target Kinesis Firehose delivery stream.
