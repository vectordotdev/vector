...
The Vector `humio_logs` sink [batches](#buffers--batches) [`log`][docs.data-model.log] events to [Humio][urls.humio] via the [HEC API][urls.humio_hec].
...
[sinks.my_sink_id]
  ...
  token = "${TOKEN_ENV_VAR}"
  healthcheck = true # default
  host = "http://myhumiohost.com" # no default
