...
component_title: "AWS SQS"
description: "The Vector `aws_sqs` sink batches `log` events to Amazon Web Service's Simple Queue Service via the `SendMessageBatch` API endpoint."
event_types: ["log"]
function_category: "transmit"
issues_url: https://github.com/timberio/vector/issues?q=is%3Aopen+is%3Aissue+label%3A%22sink%3A+aws_sqs%22
operating_systems: ["Linux","MacOS","Windows"]
sidebar_label: "aws_sqs|[\"log\"]"
source_url: https://github.com/timberio/vector/tree/master/src/sinks/aws_sqs.rs
status: "prod-ready"
title: "AWS SQS Sink"
...
The Vector `aws_sqs` sink
[batches](#buffers--batches) [`log`][docs.data-model.log] events to [Amazon Web
Service's Simple Queue Service][urls.aws_sqs] via the [`SendMessageBatch` API
endpoint](https://docs.aws.amazon.com/AWSSimpleQueueService/latest/APIReference/API_SendMessageBatch.html).
...
<!--
     ...
     website/docs/reference/sinks/aws_sqs.md.erb
-->
...
[sinks.my_sink_id]
  ...
  fifo = false # optional, default
  queue_url = "https://sqs.us-east-1.amazonaws.com/account_id/queue_name" # required
  region = "us-east-1" # required, required when endpoint = ""

  # Batch
  batch.max_events = 10 # optional, default, events
...
Set this to true if the queue is a fifo queue.
...
The queue url of the target SQS Logs queue.
...
The `aws_sqs` sink buffers & batches data as
...
with the Vector team by [opening an issue][urls.new_aws_sqs_sink_issue].
...
[urls.new_aws_sqs_sink_issue]: https://github.com/timberio/vector/issues/new?labels=sink%3A+aws_sqs
