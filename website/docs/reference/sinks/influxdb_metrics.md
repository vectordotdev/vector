...
The Vector `influxdb_metrics` sink [batches](#buffers--batches) [`metric`][docs.data-model.metric] events to [InfluxDB][urls.influxdb] using [HTTP API][urls.influxdb_http_api].
...
InfluxDB [authentication token][urls.influxdb_authentication_token]
...
[urls.influxdb_http_api]: https://v2.docs.influxdata.com/v2.0/api/#tag/Write
