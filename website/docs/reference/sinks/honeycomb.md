...
The Vector `honeycomb` sink [batches](#buffers--batches) [`log`][docs.data-model.log] events to [Honeycomb][urls.honeycomb] via the [batch events API][urls.honeycomb_batch].
...
[sinks.my_sink_id]
  # REQUIRED
  type = "honeycomb" # must be: "honeycomb"
  inputs = ["my-source-id"] # example
  api_key = "${MY_API_KEY}" # example
  dataset = "my-honeycomb-dataset" # example
