...
The Vector `datadog_logs` sink [streams](#streaming) [`log`][docs.data-model.log] events to [Datadog's][urls.datadog] logs via the [TCP endpoint][urls.datadog_logs_endpoints].
...
Datadog [API key](https://docs.datadoghq.com/api/?lang=bash#authentication) See [Obtaining a Sematext token](#obtaining-a-sematext-token) for more info.
