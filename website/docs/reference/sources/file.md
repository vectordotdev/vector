...
The Vector [`file`](#file) source ingests data through one or more local files and [outputs `log` events](#output).
...
The [`file`](#file) source ingests data through one or more local files and [outputs `log` events](#output).
For example:
...
To consume these lines as a single event with Vector use the following
configuration:
