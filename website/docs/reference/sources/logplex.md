...
protocol][urls.logplex_protocol] and [outputs `log` events](#output).
...
The `logplex` source ingests data through the [Heroku Logplex HTTP Drain protocol][urls.logplex_protocol] and [outputs `log` events](#output).
For example:
