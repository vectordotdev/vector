...
ingests data through [standard input (STDIN)][urls.stdin] and [outputs `log`
events](#output).
...
The `stdin` source ingests data through [standard input (STDIN)][urls.stdin] and [outputs `log` events](#output).
For example:
