...
ingests data through the [HTTP protocol][urls.http] and [outputs `log`
events](#output).
...
The `http` source ingests data through the [HTTP protocol][urls.http] and [outputs `log` events](#output).
For example:
