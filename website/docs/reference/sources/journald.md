...
utility and [outputs `log` events](#output).
...
The `journald` source ingests data through [Systemd's][urls.systemd] [Journald][urls.journald] utility and [outputs `log` events](#output).
For example:
