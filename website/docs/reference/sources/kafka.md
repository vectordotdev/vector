...
ingests data through [Kafka][urls.kafka] and [outputs `log` events](#output).
...
The `kafka` source ingests data through [Kafka][urls.kafka] and [outputs `log` events](#output).
For example:
