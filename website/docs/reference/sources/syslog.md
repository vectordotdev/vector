...
ingests data through the [Syslog protocol][urls.syslog_5424] and [outputs `log`
events](#output).
...
The `syslog` source ingests data through the [Syslog protocol][urls.syslog_5424] and [outputs `log` events](#output).
For example:
