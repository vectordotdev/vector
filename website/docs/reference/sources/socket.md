...
[UDP][urls.udp], or [UDS][urls.uds] socket and [outputs `log` events](#output).
...
The `socket` source ingests data through a [socket][urls.socket], such as a [TCP][urls.tcp], [UDP][urls.udp], or [UDS][urls.uds] socket and [outputs `log` events](#output).
For example:
