...
The Vector `docker` source ingests data through the [Docker engine daemon][urls.docker_daemon] and outputs [`log`][docs.data-model.log] events.
...
The `docker` source ingests data through the [Docker engine daemon][urls.docker_daemon] and outputs [`log`][docs.data-model.log] events.
For example:
...
logging driver. If another driver has been set up then Vector's docker source will be unable to collect logs
