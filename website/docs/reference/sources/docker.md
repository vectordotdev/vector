...
The Vector `docker` source ingests data through the [Docker engine daemon][urls.docker_daemon] and [outputs `log` events](#output).
...
The `docker` source ingests data through the [Docker engine daemon][urls.docker_daemon] and [outputs `log` events](#output).
For example:
