...
format][urls.prometheus_text_based_exposition_format] and [outputs `metric`
events](#output).
...
The `prometheus` source ingests data through the [Prometheus text exposition format][urls.prometheus_text_based_exposition_format] and [outputs `metric` events](#output).
For example:
