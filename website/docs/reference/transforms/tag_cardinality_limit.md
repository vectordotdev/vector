...
The Vector `tag_cardinality_limit` transform accepts and [outputs `metric` events](#output) allowing you to limit the cardinality of metric tags to prevent downstream disruption of metrics services.
...
The `tag_cardinality_limit` transform accepts and [outputs `metric` events](#output) allowing you to limit the cardinality of metric tags to prevent downstream disruption of metrics services.
For example:
