...
The Vector `json_parser` transform accepts and [outputs `log` events](#output) allowing you to parse a log field value as JSON.
...
The `json_parser` transform accepts and [outputs `log` events](#output) allowing you to parse a log field value as JSON.
For example:
