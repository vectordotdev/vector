...
The Vector `aws_ec2_metadata` transform accepts and [outputs `log` events](#output) allowing you to enrich logs with AWS EC2 instance metadata.
...
If you are running Vector within a Docker container then ensure you should pass
the `--net=host` flag when starting Docker:
