...
accepts and [outputs `log` and `metric` events](#output) allowing you to select
events based on a set of logical conditions.
...
The `filter` transform accepts and [outputs `log` and `metric` events](#output) allowing you to select events based on a set of logical conditions.
For example:
