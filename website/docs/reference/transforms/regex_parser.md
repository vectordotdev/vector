---
description: "The Vector `regex_parser` transform accepts `log` events and allows you to parse a log field's value with a Regular Expression."
event_types: ["log"]
...
The Vector `regex_parser` transform accepts [`log`][docs.data-model.log] events and allows you to parse a log field's value with a [Regular Expression][urls.regex].
