...
The Vector [`merge`](#merge) transform accepts and [outputs `log` events](#output) allowing you to merge partial log events into a single event.
...
The [`merge`](#merge) transform accepts and [outputs `log` events](#output) allowing you to merge partial log events into a single event.
For example:
