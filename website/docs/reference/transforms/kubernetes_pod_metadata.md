...
with [Kubernetes][urls.kubernetes] Pod metadata.
...
The `kubernetes_pod_metadata` transform accepts and [outputs `log` events](#output) allowing you to enrich pod's logs with [Kubernetes][urls.kubernetes] Pod metadata.
For example:
