...
# Latest ({{< version >}})
mkdir -p vector && \
  curl -sSfL --proto '=https' --tlsv1.2 https://dd-cose-releases.s3.amazonaws.com/vector/{{< version >}}/vector-{{< version >}}-aarch64-unknown-linux-musl.tar.gz | \
...
# Latest ({{< version >}})
mkdir -p vector && \
  curl -sSfL --proto '=https' --tlsv1.2 https://dd-cose-releases.s3.amazonaws.com/vector/{{< version >}}/vector-{{< version >}}-armv7-unknown-linux-gnueabihf.tar.gz | \
  tar xzf - -C vector --strip-components=2
...
mkdir -p vector && \
  curl -sSfL --proto '=https' --tlsv1.2 https://dd-cose-releases.s3.amazonaws.com/vector/{{< version >}}/vector-{{< version >}}-arm64-apple-darwin.tar.gz  | \
  tar xzf - -C vector --strip-components=2
...
```powershell
# Latest (version {{< version >}})
powershell Invoke-WebRequest https://dd-cose-releases.s3.amazonaws.com/vector/{{< version >}}/vector-{{< version >}}-x86_64-pc-windows-msvc.zip -OutFile vector-{{< version >}}-x86_64-pc-windows-msvc.zip
...
# Latest (version {{< version >}})
mkdir -p vector && \
  curl -sSfL --proto '=https' --tlsv1.2 https://dd-cose-releases.s3.amazonaws.com/vector/{{< version >}}/vector-{{< version >}}-x86_64-unknown-linux-musl.tar.gz  | \
  tar xzf - -C vector --strip-components=2
