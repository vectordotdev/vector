...
When integrating with an agent, you should configure the agent to be a simple data forwarder, and route [supported data types](docs/introduction/introduction/architecture/data-model/) through Vector. This strategy reduces the risk of data loss and service disruption due to misbehaving agents by minimizing their responsibilities.
...
Primitive data types that [Vector’s data model](docs/introduction/introduction/architecture/data-model/) supports should be processed by Vector (i.e., logs, metrics, and traces). Real-time, vendor-specific data, such as continuous profiling data, should not flow through Vector since it is not interoperable and typically does not benefit from processing.
...
- Enable [end-to-end acknowledgements](/docs/introduction/introduction/architecture/end-to-end-acknowledgements/) on all sources.
