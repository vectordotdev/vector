...
The first component uses the [`demo` source][demo], which creates sample log data that enables you to simulate different types of events in various formats.
...
You can see we've added the `sources.generated_syslog` component. The `format` option tells the demo which type of logs to emit, here `syslog`, and the `count` option tells the demo how many lines to emit, here 100.
...
[demo]: /docs/reference/configuration/sources/demo
