...
* You read the [Hello, WASM World] guide and feel comfortable with the topics it discussed.
* You've reviewed the [Unit Testing](/guides/advanced/unit-testing/) guide.

</Assumptions>

In ["Hello, WASM world"](TODO) we learnt how to get started making our first WASM plugin. In this guide, we will
learn how to go farther with that skill. We will build a plugin that merges one long message split over multiple lines,
into one message.
...
<!--
    ...
    guides/advanced/wasm-multiline.md.erb
-->
...
In the [*Hello, WASM World*](TODO) we used a test plugin called `add_fields` from Vector that did some
set up in `init`.
...
There is a full, building copy of the code at the bottom of this page.
...
As you may recall from the [Unit Testing Guide](TODO), Vector supports testing configurations right out of the box.
We'll use this to test our new WASM plugin.

<Alert type="info">

Review the [Hello, WASM world!](TODO) guide if you need a refresher on how to get a `wasm` compatible Vector during the
experimental phase.
