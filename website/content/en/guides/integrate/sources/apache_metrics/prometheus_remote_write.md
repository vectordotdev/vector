---
$schema: "../../../.schema.json"
title: "Send metrics from Apache HTTP server (HTTPD) to Prometheus"
description: "A simple guide to send metrics from Apache HTTP server (HTTPD) to Prometheus in just a few minutes."
author_github: https://github.com/binarylogic
cover_label: "Apache HTTP Server (HTTPD) Metrics to Prometheus Remote Write Integration"
tags: ["type: tutorial","domain: sources","domain: sinks","source: apache_metrics","sink: prometheus_remote_write"]
...
send metrics from Apache HTTP server (HTTPD) to Prometheus.
...
<!--
    ...
    guides/integrate/sources/apache_metrics/prometheus_remote_write.md.erb
-->
...
[Prometheus](https://prometheus.io/) is a pull-based monitoring system that scrapes metrics from configured endpoints, stores them efficiently, and supports a powerful query language to compose dynamic information from a variety of otherwise unrelated data points.
...
<DaemonDiagram
  platformName={null}
  sourceName={"apache_metrics"}
  sinkName={"prometheus_remote_write"} />
...
<div className="list--icons list--icons--checks list--indent">
  ...
  * Collects metrics from [Apache HTTP server (HTTPD)](https://httpd.apache.org).
    * Enriches data with useful Apache HTTP server (HTTPD) context.
    * Efficiently collects data and checkpoints read positions to ensure data is not lost between restarts.
  * Sends metrics to [Prometheus](https://prometheus.io/).
...
<Tabs
  ...
  groupId="integration-guide-apache_metrics.prometheus_remote_write_interface"
