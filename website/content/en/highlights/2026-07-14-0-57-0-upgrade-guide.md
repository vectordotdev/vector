...
`${VAR}`-based configs by interpolating them yourself with a pre-processing tool like [`envsubst`](https://man7.org/linux/man-pages/man1/envsubst.1.html)
before handing the result to Vector. See the [environment variables reference](/docs/reference/environment_variables/#security-restrictions)
for more details.

If you're using env var interpolation specifically to inject secrets (API keys, passwords, tokens), the
...
An unconfined template will make the command fail validation. Note that `vector validate --no-environment` skips
this check; drop the flag if you want confinement errors to be caught.
