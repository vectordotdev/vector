...
{{< info >}}
Vector `0.51.0` includes several breaking changes:
{{< /info >}}
...
6. [VRL `find` function return value change](#vrl-find-function)
...
If you need to inject multi-line configuration blocks, use a config pre-processing tool like `envsubst` instead.
