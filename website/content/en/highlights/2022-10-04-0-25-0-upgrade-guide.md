...
1. [`internal_metrics` defaults to setting `host` tag](#internal_metrics-host_tag)
...
#### `internal_metrics` defaults to setting `host` tag {#internal_metrics-host_tag)
...
without opting into this tag via `tags.host_key`. The default key for this the configured
