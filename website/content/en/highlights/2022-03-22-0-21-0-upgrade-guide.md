...
9. [Route transform has now an `_unmatched` output](#transform-route-unmatched)
...
#### Route transform has now an `_unmatched` output {#transform-route-unmatched}
...
Those non matching events can now be used through the `_unmatched` output like in the following example.
...
[sinks.bar]
type = "..."
inputs = ["foo._unmatched"]
```

As a side effect, the `event_discarded_total` metric emitted by the `route` transform is now gone considering the events
are no longer discarded and can now be caught using the `_unmatched` output.
