...
2. [New metrics behavior for `route` transform](#metrics-route-transform)
3. [New internal events for `loki` sink](#events-loki-sink)
4. [`if` statement predicates in `VRL` need to handle errors](#vrl-fallible-predicates)
5. [Division by a literal nonzero number in `VRL` is no longer fallible](#vrl-infallible-division)
6. ['syslog' decoder will raise an error if the incoming message cannot be decoded successfully](#syslog-decoder)
7. [Unit tests no longer allow nonexistent targets in `extract_from`/`no_outputs_from` configurations](#unit-test-nonexistent-build-error)
