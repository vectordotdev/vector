...
3. [VRL's `parse_syslog` now nests structured data fields](#vrl-parse_syslog)
...
#### VRL's `parse_syslog` now nests structured data fields {#vrl-parse_syslog}

The `parse_syslog` VRL function now returns the structured data elements as a nested map under the
structured data element name, rather than prefixing each key with the element name, to match the
behavior of the `syslog` source.
...
The previous behavior can be acheived by running the result through the
`flatten` function like:
