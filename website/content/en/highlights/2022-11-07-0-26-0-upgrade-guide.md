...
The `vector` source was not enriching events with the `source_type` and `timestamp` fields.
This behavior was added, using the global options [log_schema.source_type_key](https://vector.dev/docs/reference/configuration/global-options/#log_schema.source_type_key)
and [log_schema.timestamp_key](https://vector.dev/docs/reference/configuration/global-options/#log_schema.timestamp_key),
for consistency.

This marks a breaking change to the shape of the log events output from the `vector` source.
