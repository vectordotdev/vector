...
### Removal of deprecated configuration fields for the Splunk HEC Logs sink: `host` and `token` {#splunk-hec-logs-sink-deprecated-fields}

We've removed a long deprecated configuration field from the Splunk HEC Logs
sink: `host`. 

Additionally, the `token` field has been renamed to `default_token` in support for passthrough token routing.

You can migrate your configuration by switching to `endpoint` and `token` respecitively.
