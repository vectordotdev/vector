...
3. [`http_client_responses_total` now labels status with only numeric code](#http_client_responses_total)
4. [Field name change for aggregated summaries in `metric_to_log` transform](#agg_summary_metric_to_log)
5. [Removal of deprecated configuration fields for the Datadog metrics sink: `host` and `namespace`](#dd_metrics_sink_deprecated_fields)
