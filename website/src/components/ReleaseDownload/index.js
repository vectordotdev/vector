...
function Downloads({ browsePath, downloads }) {
  ...
  return (
    <ul className={styles.downloadFiles}>
              ...
              {groupedDownloads[key].map((download, downloadIdx) => (
                <li key={downloadIdx}>
                  <a
                    ...
                    href={`https://packages.timber.io/vector/${browsePath}/${download.file_name}`}
                  >
      ...
      <li>
        <a
          href={`https://packages.timber.io/vector/${browsePath}/`}
          target="_blank"
        >
...
function DownloadTable({
  ...
  const {
    metadata: { installation: installation, latest_release: latestRelease },
  ...
  return (
    <div className={styles.downloadTable}>
      <div>
        ...
        <div>
          ...
          {releaseNotesPath && (
            <>
              {" "}
              • <Link to={releaseNotesPath}>release notes</Link>
            </>
          )}
        </div>
      </div>
      <div>
        ...
        <div>
          <a
            href="https://github.com/timberio/vector/blob/master/LICENSE"
            target="_blank"
          >
            Apache 2
          </a>
      ...
      <div>
        ...
        <div>
          {Object.values(platforms).map((platform, idx) => (
            <span key={idx}>
              {idx > 0 ? " • " : ""}
              <Link to={`/docs/setup/installation/platforms/${platform.name}/`}>
                {" "}
                {platform.title}
              </Link>
            </span>
        ...
        <div>
          {Object.values(packageManagers).map((packageManager, idx) => (
            <span key={idx}>
              {idx > 0 ? " • " : ""}
              <Link
                to={`/docs/setup/installation/package-managers/${packageManager.name}/`}
              >
                {packageManager.title}
              </Link>
            </span>
          ))}
        </div>
...
function ReleaseDownload({ version }) {
  ...
  const release = releasesObj[selectedVersion];
  const releases = Object.values(releasesObj).slice(0).reverse();
        ...
        <section>
          ...
          >
            ...
            <Jump to="/guides/getting-started/">
              <i className="feather icon-book-open"></i> Follow the getting
              started guide
            </Jump>
          </div>
        </section>
