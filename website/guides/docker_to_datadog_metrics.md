...
keywords: ["docker","datadog metrics"]
---

In this guide we'll be consuming `docker` logs and writing them to a `datadog_metrics` sink.
...
`docker` source into metrics. We can do that with a
