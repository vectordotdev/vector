...
All of these fields are set when data is ingested (in a [source][docs.sources]). It may be that your data does not
...
If your vector pipeline is only consuming data from these sources, you can add the following to your `vector.toml`:
...
application can't send EU user data to our US based kafka.
...
[`add_field`][transform_add_fields] transform that you can use alongside the
[`remove_fields`][transform_remove_fields] transform to do just that!
...
What if you had to do this in reverse? Try using the [`regex`][transform_regex_parser] or
[`split`][transform_split] transforms.
...
The [`coercer`][transform_coercer] transform is the correct tool for this job!
...
You can also use a transform like [`json_parser`][transform_json_parser] or
[`grok_parser`][transform_grok_parser] to parse out data in a given field.
