...
tags: ["type: guide", "domain: transforms"]
---

While Vector's built-in [transforms][docs.reference.transforms] are fast, sometimes they are not are not expressive
enough for you needs. In such cases the [`lua`][docs.reference.transforms.lua] transform comes to rescue, letting you
...
of Lua interpreter. As a consequence, different transforms are isolated and cannot interrupt each other.
...
However, this code could and should be refactored. Hold on to the next section to see way in which it could be done.
...
With this approach the config from the previous section becomes splitted into two files:
