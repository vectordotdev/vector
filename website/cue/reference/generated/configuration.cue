	...
	provider: {
		type: object: options: {
			config_format: {
				type: string: {
					enum: {
						toml: "TOML format is used."
						json: "JSON format is used."
						yaml: "YAML format is used."
					}
					default: "toml"
				}
				description: "Which config format expected to be loaded"
				required:    false
			}
			interpolate_env: {
				type: bool: default: false
				description: "Enable environment variable interpolation"
				required:    false
			}
			poll_interval_secs: {
				type: uint: default: 30
