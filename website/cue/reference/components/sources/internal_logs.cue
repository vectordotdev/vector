...
components: sources: internal_logs: {
	...
	how_it_works: {
		limited_logs: {
			title: "Logs are limited by the `level` option"
			body: """
				The selection of log messages delivered to this source is determined by its `level`
				option, which defaults to `info`. It is independent of the console log level set at
