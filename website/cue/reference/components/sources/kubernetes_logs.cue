	...
	configuration: {
		...
		delay_deletion_ms: {
			...
			description: """
				...
				If Vector tries to process logs from a Pod which has already had it's metadata removed from the local cache, it will fail to enrich the event with metadata and log a warning.
				"""
			required:    false
			type: uint: {
				default: 60_000
