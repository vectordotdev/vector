	...
	how_it_works: {
		advantages: {
			...
			body: """
				...
				accurately for components like the File sink, where some files might end up missing. Second, it can reduce 
				overhead for downstream components like Prometheus Remote Write, which internally convert incremental to 
				absolute metrics. Converting to absolute metric with this transform prevents the creation of duplicate caches
				when sending to multiple Prometheus Remote Write sinks.

				The conversion is performed based on the order in which incremental metrics are received, not their timestamps. 
				Moreover, absolute metrics received by this sink are emitted unchanged.
				"""
		}
	}
}
