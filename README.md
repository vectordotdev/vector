...
* You _SHOULD_ use Vector to replace Logstash, Fluentd, Fluentbit, or similar tools.
* You _SHOULD_ use Vector as an [application sidecar][docs.roles.agent].
...
* **Metrics aggregation** - Aggregated [histgorams][docs.data-model.metric#aggregated_histogram] and [summaries][docs.data-model.metric#aggregated_summary] aggregate to reduce volume without loss of precision.
...
* **Dynamic partitioning** - Create dynamic partitions on the fly ([ex][docs.sinks.aws_s3#partitioning]) with Vector's [templating syntax][docs.reference.templating].
...
* **Backpressure & load shedding** - Buffers can be configured to provide backoffpressure or shed load.
