    ...
    pub fn resolve_enum_schema(&mut self, schema: &Value) -> Result<Value> {
        ...
        if enum_tagging == "untagged" {
            ...
            for subschema in &subschemas {
                ...
                if self.resolved_schema_type(&resolved) == Some("object") {
                    resolved_objects.push((subschema.clone(), resolved));
                } else {
                    all_objects = false;
                    break;
                }
            ...
            if all_objects && resolved_objects.len() >= 2 {
                // Only treat the variants as mutually exclusive groups when their fields are
                // disjoint (e.g. a flattened enum of settings structs). Overlapping variants
                // (e.g. the lua transform's v1/v2 configs) fall through to the regular merge.
                let mut seen_fields: HashSet<String> = HashSet::new();
                let mut disjoint = true;
                for (_, resolved_subschema) in &resolved_objects {
                    let opts = resolved_subschema
                        .pointer("/type/object/options")
                        .and_then(Value::as_object)
                        .expect("resolved object variant must have options");
                    for prop_name in opts.keys() {
                        if !seen_fields.insert(prop_name.clone()) {
                            disjoint = false;
                            break;
                        }
                    }
                ...
                if disjoint {
                    debug!("Resolved as 'untagged with disjoint object variants' enum schema.");
                    let mut unique_resolved_properties = Map::new();
                    for (subschema, resolved_subschema) in &resolved_objects {
                        let variant_name = get_schema_metadata(subschema, "logical_name")
                        ...
                        let opts = resolved_subschema
                            ...
                            .and_then(Value::as_object)
                            .expect("resolved object variant must have options");
                        for (prop_name, prop_schema) in opts {
                            let mut prop = prop_schema.clone();
                            prop.as_object_mut().unwrap().insert(
                                "groups".to_string(),
                                Value::Array(vec![Value::String(variant_name.clone())]),
                            );
                            unique_resolved_properties.insert(prop_name.clone(), prop);
                        }
                    }
                    return Ok(
                        json!({ "_resolved": { "type": { "object": { "options": unique_resolved_properties } } } }),
