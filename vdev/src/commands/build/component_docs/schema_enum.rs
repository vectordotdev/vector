use super::{get_schema_metadata, schema_aware_nested_merge, SchemaContext};
use anyhow::Result;
...
impl SchemaContext {
    ...
    pub fn resolve_enum_schema(&mut self, schema: &Value) -> Result<Value> {
        let mut subschemas = if let Some(one) = schema.get("oneOf") {
            ...
            error!("Enum schema had both `oneOf` and `anyOf` specified (or neither). Schema: {}", schema);
            std::process::exit(1);
        };
        ...
        if enum_tagging.is_none() {
            error!("Enum schemas should never be missing the metadata for the enum tagging mode. Schema: {}", schema);
            std::process::exit(1);
        }
        let enum_tagging = enum_tagging.unwrap();
        let enum_tag_field = get_schema_metadata(schema, "docs::enum_tag_field").and_then(|v| v.as_str());
        ...
        if subschema_count == 2 {
            ...
            if let Some(idx) = array_idx {
                ...
                if Some(&single_reduced) == array_reduced.get("items") {
                    ...
                    if self.get_json_schema_type(&single_subschema) == schema.get("default").map(|d| super::json_type_str(d)) {
                        single_subschema.as_object_mut().unwrap().insert("default".to_string(), schema.get("default").unwrap().clone());
                    }

                    let resolved_subschema = self.resolve_schema(&single_subschema)?;
                    debug!("Resolved as 'X or array of X' enum schema.");
                    return Ok(json!({ "_resolved": resolved_subschema, "annotations": "single_or_array" }));
                }
            }
        }
        ...
        if enum_tagging == "internal" {
            ...
            if resolved_subschemas.len() == subschema_count {
                ...
                for resolved_subschema in &mut resolved_subschemas {
                    ...
                    let opts = resolved_subschema.pointer_mut("/type/object/options").unwrap().as_object_mut().unwrap();
                    let mut tag_subschema = opts.remove(tag_field).unwrap();

                    if let Some(t) = title {
                        tag_subschema.as_object_mut().unwrap().insert("title".to_string(), t);
                    }
                    if let Some(d) = desc {
                        tag_subschema.as_object_mut().unwrap().insert("description".to_string(), d);
                    }
                    ...
                    if tag_value.is_none() {
                        error!("All enum subschemas representing an internally-tagged enum must have the tag field use a const value.");
                        std::process::exit(1);
                    }
                    let tag_val_str = tag_value.unwrap();

                    if unique_tag_values.contains_key(&tag_val_str) {
                        error!("Found duplicate tag value '{}' when resolving enum subschemas.", tag_val_str);
                        std::process::exit(1);
                    }
                    ...
                    for (prop_name, prop_schema) in opts.iter_mut() {
                        if let Some(existing) = unique_resolved_properties.get_mut(prop_name) {
                            ...
                            if reduced_existing != reduced_new {
                                error!("Had overlapping property '{}' from resolved enum subschema, but schemas differed.", prop_name);
                                std::process::exit(1);
                            }
                            existing.get_mut("relevant_when").unwrap().as_array_mut().unwrap().push(Value::String(tag_val_str.clone()));
                        } else {
                            prop_schema.as_object_mut().unwrap().insert("relevant_when".to_string(), json!([tag_val_str.clone()]));
                            unique_resolved_properties.insert(prop_name.clone(), prop_schema.clone());
                        }
                    }
                }
                ...
                resolved_tag_property_obj.insert("type".to_string(), json!({ "string": { "enum": enum_vals } }));

                let tag_desc = get_schema_metadata(schema, "docs::enum_tag_description");
                if tag_desc.is_none() {
                    error!("A unique tag description must be specified for enums which are internally tagged");
                    std::process::exit(1);
                }
