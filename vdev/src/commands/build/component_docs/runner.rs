...
/// Extracts variant descriptions from a metric enum JSON schema (oneOf / enum array)
/// and returns an iterator of `(metric_name, description)` pairs.
fn extract_metric_variants(schema: &Value) -> impl Iterator<Item = (&str, &str)> {
    schema
        .get("oneOf")
        .and_then(Value::as_array)
        .into_iter()
        .flatten()
        .filter_map(|variant| {
            let name = variant.get("const").and_then(Value::as_str)?;
...
fn generate_internal_metric_descriptions(metric_schemas: &Value) -> Result<()> {
    ...
    for (metric_type, schema) in [
        ...
        if let Some(schema) = schema {
            for (name, desc) in extract_metric_variants(schema) {
                // Escape backslashes and double-quotes for CUE string literals.
                let escaped = desc.replace('\\', "\\\\").replace('"', "\\\"");
                lines.push((name.to_string(), escaped));
                let _ = metric_type; // type is encoded in which enum the variant came from
            }
        }
    }
    ...
    for (name, desc) in &lines {
        cue.push_str(&format!("\t{name}: description: \"{desc}\"\n"));
    }
    cue.push_str("}\n");
