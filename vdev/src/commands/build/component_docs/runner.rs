...
pub fn run(schema_path: &Path) -> Result<()> {
    ...
    let root_schema: Value = serde_json::from_str(&schema_content)
    ...
    let mut all_components: std::collections::HashMap<String, serde_json::Map<String, Value>> = std::collections::HashMap::new();
    if let Some(definitions) = root_schema.get("definitions").and_then(|d| d.as_object()) {
        for (key, definition) in definitions {
    ...
    for (comp_type, components) in all_components {
        for (comp_name, schema_name) in components {
            render_and_import_component_schema(&mut context, schema_name.as_str().unwrap(), &comp_type, &comp_name)?;
        }
    }
...
fn write_to_temp_file(prefix: &str, suffix: &str, content: &str) -> Result<PathBuf> {
    use std::io::Write;
    let mut tmp = tempfile::Builder::new().prefix(prefix).suffix(suffix).tempfile()?;
...
fn render_and_import_schema(
    ...
    for segment in config_map_path {
        ...
        current_obj = current_obj.get_mut(*segment).unwrap().as_object_mut().unwrap();
    }
    ...
    info!("[✓]   Wrote {} schema to '{}'. ({} bytes)", friendly_name, json_output_file.display(), final_json.len());
    ...
    let status = Command::new(&context.cue_binary_path)
        .args(["import", "-f", "-o", cue_output_file.to_str().unwrap(), "-p", "metadata", json_output_file.to_str().unwrap()])
        .status()?;

    if !status.success() {
        error!("[!]   Failed to import {} schema as valid Cue.", friendly_name);
        std::process::exit(1);
    }

    info!("[✓]   Imported {} schema to '{}'.", friendly_name, cue_output_file.display());
    Ok(())
}
...
fn render_and_import_generated_api_schema(context: &mut SchemaContext, apis: serde_json::Map<String, Value>) -> Result<()> {
...
fn render_and_import_generated_global_option_schema(context: &mut SchemaContext, global_options: serde_json::Map<String, Value>) -> Result<()> {
