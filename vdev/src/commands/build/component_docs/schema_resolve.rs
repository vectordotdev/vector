...
impl SchemaContext {
    ...
    pub fn resolve_bare_schema(&mut self, schema: &Value) -> Result<Value> {
        ...
        let res = match schema_type {
            ...
            Some("one-of" | "any-of") => {
                // Skip pure `required`-only constraint schemas generated by required_one_of.
                // These encode structural constraints like {"oneOf":[{"required":["a"]},{"required":["b"]}]}
                // and are not enum definitions; treating them as enums would fail.
