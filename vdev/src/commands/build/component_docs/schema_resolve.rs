...
impl SchemaContext {
    ...
    pub fn resolve_bare_schema(&mut self, schema: &Value) -> Result<Value> {
        // Constraint schemas generated by exclusive field groups express a cross-field requirement
        // rather than any field's own type. They are not enum definitions and have no instance type
        // of their own, so resolving them would either fail enum resolution or fall through to the
        // unconstrained case and add a spurious wildcard option.
        if [
            "_required_one_of_constraint",
