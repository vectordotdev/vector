...
impl Git {
    ...
    pub fn changed_files(&self) -> Result<Vec<String>> {
        ...
        let output = self.output(&["diff", "--name-status", "origin/master..."])?;
        for line in output.lines() {
            if !self.is_warning_line(line) {
        ...
        // Tracked
        let output = self.output(&["diff", "--name-only", "HEAD"])?;
        for line in output.lines() {
            if !self.is_warning_line(line) {
        ...
        // Untracked
        let output = self.output(&["ls-files", "--others", "--exclude-standard"])?;
        for line in output.lines() {
            files.insert(line.to_string());
        }
