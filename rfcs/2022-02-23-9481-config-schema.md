...
- How can we provide logical constraints between disparate components? For example, using disk
  buffers implies needing a global data directory set, but there's not really a way in JSONSchema to
  add a conditional constraint to a remote definition/schema.
