...
I'm not 100% sure this is entirely required or accurate but will require some testing as we proceed.

The query will return these metrics by parsing the query results and converting them into metrics. This list is the default JVM metrics available via JMX. Individual applications (Kafka, Cassandra, Tomcat, etc) will expose their own MBeans and metrics.
...
- `java_nio_BufferPool_Count tagged with `name` (untyped)
...
- `java_nio_BufferPool_MemoryUsed tagged with `name` (untyped)
- `java_nio_BufferPool_TotalCapacity tagged with `name` (untyped)
...
[sources.my_source_id]
  ...
  namespace = "java" # optional, default is "java", namespace to put metrics under
```
