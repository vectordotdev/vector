...
use crate::config::{ComponentKey, ConfigBuilder, TestDefinition};
...
fn deserialize_component_map<T>(
    map: ConfigMap,
    hint: ComponentHint,
) -> Result<IndexMap<ComponentKey, T>, Vec<String>>
where
    T: DeserializeOwned,
{
    deserialize_config_map_wrapped(map, hint.as_component_field())
}
