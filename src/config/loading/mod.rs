...
pub(crate) async fn load_builder_from_paths_with_secrets(
    ...
    let secrets_backends_loader = load_secret_backends_from_paths(config_paths)?;
    let secrets = retrieve_referenced_secrets(secrets_backends_loader, signal_handler).await?;
    let mut builder = load_builder_from_paths_with_resolved_secrets(config_paths, secrets)?;
...
async fn retrieve_referenced_secrets(
    mut loader: SecretBackendLoader,
    signal_handler: &mut signal::SignalHandler,
) -> Result<HashMap<String, String>, Vec<String>> {
    if !loader.has_secrets_to_retrieve() {
...
/// Uses `ConfigBuilderLoader` to load a `ConfigBuilder` from configuration paths.
...
/// Loads a `ConfigBuilder` from paths and replaces secret placeholders.
pub fn load_builder_from_paths_with_resolved_secrets(
    config_paths: &[ConfigPath],
...
/// Uses `SecretBackendLoader` to load secret backends from configuration paths.
