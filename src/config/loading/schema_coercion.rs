...
fn coerce_any_of(
    ...
    for prefer_matching_type in [true, false] {
        for schema in schemas {
            if schema_matches_value_type(schema, definitions, value) != prefer_matching_type {
                continue;
            }
...
fn schema_matches_value_type(schema: &Value, definitions: Option<&Value>, value: &Value) -> bool {
    if let Some(reference) = schema.get("$ref").and_then(Value::as_str)
        && let Some(key) = reference.strip_prefix("#/definitions/")
        && let Some(schema) = definitions.and_then(|definitions| definitions.get(key))
    {
        return schema_matches_value_type(schema, definitions, value);
    }
    ...
    ["allOf", "anyOf", "oneOf"].iter().any(|keyword| {
        schema
            ...
            .is_some_and(|schemas| {
                schemas
                    .iter()
                    .any(|schema| schema_matches_value_type(schema, definitions, value))
            })
    })
}
