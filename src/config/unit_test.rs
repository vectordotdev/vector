...
async fn build_unit_tests(mut config_builder: ConfigBuilder) -> Result<Vec<UnitTest>, Vec<String>> {
    let mut test_definitions = std::mem::take(&mut config_builder.tests);
...
fn build_and_validate_inputs(test_inputs: &Vec<TestInput>, available_insert_targets: &HashSet<ComponentKey>) -> Result<Vec<(ComponentKey, Event)>, Vec<String>> {
    ...
    for (index, input) in test_inputs.iter().enumerate() {
        if available_insert_targets.contains(&input.insert_at) {
          ...
          errors.push(format!("inputs[{}]: unable to locate target transform '{}'", index, input.insert_at))
        }
    }
