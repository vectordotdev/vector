...
impl TapController {
    ...
    pub fn new(watch_rx: WatchRx, tap_tx: TapSender, component_patterns: &[String]) -> Self {
        let (_shutdown, shutdown_rx) = oneshot::channel();

        tokio::spawn(tap_handler(
            component_patterns.iter().cloned().collect(),
            tap_tx,
            watch_rx,
            shutdown_rx,
        ));
...
async fn tap_handler(
    component_patterns: HashSet<String>,
    tx: TapSender,
    mut watch_rx: WatchRx,
    mut shutdown_rx: ShutdownRx,
) {
    debug!(message = "Started tap.", patterns = ?component_patterns);
    ...
    loop {
        tokio::select! {
            ...
            Ok(_) = watch_rx.changed() => {
                ...
                let last_matches = component_patterns
                    .iter()
                        ...
                        .collect_vec()
                    {
                        found if !found.is_empty() => {
                            debug!(
                                ...
                                ?component_key, ?component_patterns, matched = ?found
                            );
                        ...
                        _ => {
                            debug!(
                                message="Component not matched.", ?component_key, ?component_patterns
                            );
                        }
                    }
                }
                ...
                let mut notifications = Vec::with_capacity(component_patterns.len());
                ...
                for pattern in HashSet::from_iter(&component_patterns).difference(&matched) {
    ...
    debug!(message = "Stopped tap.", patterns = ?component_patterns);
}
