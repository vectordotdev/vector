...
impl observability::Service for ObservabilityService {
    ...
    async fn get_components(
        ...
        use crate::proto::observability::{Component as ProtoComponent, ComponentType};
    ...
    async fn stream_output_events(
        ...
        use vector_lib::tap::controller::{TapController, TapPatterns};
...
fn tap_payload_to_output_events(
    payload: vector_lib::tap::controller::TapPayload,
    ...
    match payload {
        vector_lib::tap::controller::TapPayload::Log(output, log_array) => log_array
            .into_iter()
            .map(|log_event| {
                ...
                OutputEvent {
                    event: Some(output_event::Event::Log(
                        crate::proto::observability::LogEvent {
            ...
            .collect(),
        vector_lib::tap::controller::TapPayload::Metric(output, metric_array) => metric_array
            .into_iter()
            .map(|metric_event| {
                ...
                OutputEvent {
                    event: Some(output_event::Event::Metric(
                        crate::proto::observability::MetricEvent {
                            component_id: output.output_id.component.id().to_string(),
                            component_type: output.component_type.to_string(),
                            component_kind: output.component_kind.to_string(),
                            timestamp,
            ...
            .collect(),
        vector_lib::tap::controller::TapPayload::Trace(output, trace_array) => trace_array
            .into_iter()
            .map(|trace_event| {
                let encoded_string = encode_trace_event(&trace_event, encoding);

                OutputEvent {
                    event: Some(output_event::Event::Trace(
                        crate::proto::observability::TraceEvent {
                            component_id: output.output_id.component.id().to_string(),
                            component_type: output.component_type.to_string(),
                            component_kind: output.component_kind.to_string(),
                            encoded_string,
            ...
            .collect(),
        vector_lib::tap::controller::TapPayload::Notification(notification) => {
            vec![create_notification_event(notification.as_str())]
        }
    }
}

fn encode_log_event(log_event: &vector_lib::event::LogEvent, encoding: EventEncoding) -> String {
...
fn encode_metric_event(
    metric_event: &vector_lib::event::Metric,
    encoding: EventEncoding,
) -> String {
    match encoding {
...
fn encode_trace_event(
    trace_event: &vector_lib::event::TraceEvent,
    encoding: EventEncoding,
) -> String {
    match encoding {
...
fn create_notification_event(message: &str) -> OutputEvent {
    OutputEvent {
        event: Some(output_event::Event::Notification(
            crate::proto::observability::EventNotification {
