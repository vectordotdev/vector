use std::{cell::RefCell, sync::atomic::{AtomicUsize, Ordering}, num::NonZeroUsize};
...
thread_local! {
    ...
    pub(crate) static CURRENT_ALLOCATION_GROUP: RefCell<AllocationGroupId> = RefCell::new(AllocationGroupId::ROOT);
}
...
impl AllocationGroupId {
	...
	pub(super) const fn from_raw(raw_group_id: usize) -> Self {
		Self(unsafe { NonZeroUsize::new_unchecked(raw_group_id) })
	}
...
pub(super) struct AllocationGroupToken {
...
impl AllocationGroupToken {
	pub(super) fn enter(&mut self) {
		if self.previous.is_some() {
			panic!("Should not be entering a token which has already been entered: previous={}, id={}", self.previous.unwrap().as_raw(), self.id.as_raw());
		}
	...
	pub(super) fn exit(&mut self) {
		match self.previous.take() {
			None => panic!("Should not be exiting a token which has not yet been entered: id={}", self.id.as_raw()),
			Some(previous) => {
				let current = set_current_allocation_group(previous);
				if self.id != current {
					panic!("Expected current allocation group for thread to be {}, but got {}.", self.id.as_raw(), current.as_raw());
				}
			},
		}
	}
...
fn set_current_allocation_group(group_id: AllocationGroupId) -> AllocationGroupId {
	...
	CURRENT_ALLOCATION_GROUP.try_with(|current_group_id| current_group_id.replace(group_id))
...
{
	CURRENT_ALLOCATION_GROUP.try_with(|current_group_id| {
