...
use crate::{
    ...
    event::{self, LogEvent, Metric, MetricKind, MetricValue, Value},
    internal_events::{
...
impl_generate_config_from_default!(DockerStatsConfig);

#[async_trait::async_trait]
impl SourceConfig for DockerStatsConfig {
    async fn build(&self, cx: SourceContext) -> crate::Result<Source> {
        let source = DockerStatsSource::new(
        ...
        // Once this ShutdownSignal resolves it will drop DockerStatsSource and by extension it's ShutdownSignal.
...
impl DockerStatsSourceCore {
    fn new(config: DockerStatsConfig) -> crate::Result<Self> {
...
/// Where each event_stream corresponds to a running container marked with DockerStatsInfo.
/// While running, event_stream streams Events to out channel.
/// Once a log stream has ended, it sends DockerStatsInfo back to main.
///
...
struct DockerStatsSource {
    ...
    ///receives DockerStatsInfo coming from event stream futures
    main_recv: mpsc::UnboundedReceiver<Result<DockerStatsInfo, (ContainerId, ErrorPersistence)>>,
...
impl DockerStatsSource {
    fn new(
        ...
        out: SourceSender,
        shutdown: ShutdownSignal,
    ) -> crate::Result<DockerStatsSource> {
        ...
        // Only logs created at, or after this moment are logged.
        let core = DockerStatsSourceCore::new(config)?;
        ...
        let (main_send, main_recv) =
            mpsc::unbounded_channel::<Result<DockerStatsInfo, (ContainerId, ErrorPersistence)>>();
                    ...
                    match value {
                        Some(Ok(info)) => {
                            let state = self
                                .containers
                                .get_mut(&info.id)
                                .expect("Every DockerStatsInfo has it's ContainerState");
...
struct EventStreamBuilder {
    host_key: String,
    hostname: Option<String>,
    core: Arc<DockerStatsSourceCore>,
    /// Event stream futures send events through this
    out: SourceSender,
    /// End through which event stream futures send DockerStatsInfo to main future
    main_send: mpsc::UnboundedSender<Result<DockerStatsInfo, (ContainerId, ErrorPersistence)>>,
...
impl EventStreamBuilder {
            ...
            async move {
                    ...
                    .await
                {
                    Ok(details) => match ContainerMetadata::from_details(details) {
                        Ok(metadata) => {
                            let info = DockerStatsInfo::new(id, metadata, this.core.now_timestamp);
    ...
    async fn run_event_stream(mut self, info: DockerStatsInfo) {
        ...
        let container = info.id.clone();
        let container_id = container.as_str();
        let stream = self.core.docker.stats(container_id, options);
        emit!(DockerLogsContainerWatch { container_id });
        ...
        let events_stream = stream
            .map(|value| {
                match value {
                    Ok(stats) => Ok(into_metrics(stats, &bytes_received)),
                    Err(error) => {
            ...
            .flat_map(|v| futures::stream::iter(v.unwrap()))
            .take_until(self.shutdown.clone());

        let mut events_stream: Box<dyn Stream<Item = Metric> + Unpin + Send> =
            Box::new(events_stream);

        let host_key = self.host_key.clone();
        let hostname = self.hostname.clone();
        let result = {
            // let mut stream = events_stream.flat_map(|list| futures::stream::iter(list));
            self.out
                .send_event_stream(&mut events_stream)
                .await
                .map_err(|error| {
                    let (count, _) = events_stream.size_hint();
                    emit!(StreamClosedError { error, count });
                })
        };

        // End of stream
        emit!(DockerLogsContainerUnwatch { container_id });
    ...
    fn finish(self, result: Result<DockerStatsInfo, (ContainerId, ErrorPersistence)>) {
...
impl ContainerState {
    /// It's DockerStatsInfo pair must be created exactly once.
    ...
    #[must_use]
    fn return_info(&mut self, info: DockerStatsInfo) -> bool {
    ...
    fn take_info(&mut self) -> Option<DockerStatsInfo> {
...
fn into_metrics(stats: Stats, bytes_received: &Registered<BytesReceived>) -> Vec<Metric> {
    bytes_received.emit(ByteSize(std::mem::size_of_val(&stats)));

    let mut result = Vec::default();

    result.push(Metric::new(
        "docker.stats.num_procs",
        MetricKind::Absolute,
        MetricValue::Gauge {
            value: stats.num_procs as f64,
        },
    ));
    ...
    emit!(DockerLogsEventsReceived {
        byte_size: result.size_of(),
        container_id: stats.id.as_str(),
        container_name: stats.name.as_str(),
    });
...
mod tests {
    ...
    #[test]
    fn generate_config() {
        crate::test_util::test_generate_config::<DockerStatsConfig>();
    }
    ...
    fn exclude_self() {
        ...
        let mut source =
            DockerStatsSource::new(DockerStatsConfig::default(), tx, ShutdownSignal::noop())
                .unwrap();
...
#[cfg(all(test, feature = "docker-stats-integration-tests"))]
mod integration_tests {
    ...
    fn source_with<'a, L: Into<Option<&'a str>>>(
        ...
        source_with_config(DockerStatsConfig {
            ...
            ..DockerStatsConfig::default()
        })
    }

    fn source_with_config(config: DockerStatsConfig) -> impl Stream<Item = Event> {
    ...
    async fn exclude_containers() {
        ...
        let out = source_with_config(DockerStatsConfig {
    ...
    async fn not_include_image() {
        ...
        let config_ex = DockerStatsConfig {
    ...
    async fn not_include_running_image() {
        ...
        let config_in = DockerStatsConfig {
            include_containers: Some(vec![name.to_owned()]),
            include_images: Some(vec!["busybox".to_owned()]),
            ..DockerStatsConfig::default()
        };
