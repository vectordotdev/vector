...
impl StreamDecodingError for DecodeError {
    fn can_continue(&self) -> bool {
        ...
        match self {
            ...
            // A premature WindowSize means the sender desynced from the window
            // contract; the stream can't be trusted, so close the connection.
            PrematureWindowSize { .. } => false,
        }
    }
}
    ...
    fn decode(&mut self, src: &mut BytesMut) -> Result<Option<Self::Item>, Self::Error> {
        ...
        loop {
            self.state = match self.state {
                ...
                // in this window before waiting for an ACK. We count those
                // events down so the acker can mark the window's final frame and
                // emit exactly one ACK per completed window, even when ReadyFrames
                // batches several windows together.
                //
                // The protocol spec defines window size as a *maximum* unacked
                // count, which would let a writer underfill a window. In practice
                // every real client (go-lumber, beats) sets the window size to the
                // exact number of events it then sends, and the reference server
                // treats it as exact. We rely on that observed behavior, not the
                // looser spec wording. See the "Window size" section of
                // logstash.md for the client/server references.
                //
                ...
                LogstashDecoderReadState::ReadFrame(_protocol, LogstashFrameType::WindowSize) => {
                    // A new window must not open until the current one has
                    // received all advertised events. Because real clients always
                    // fill a window exactly (see above), a `WindowSize` arriving
                    // mid-window means the sender has desynced from the window
                    // contract; reject it as fatal (matching the reference server)
                    // rather than guess an ACK boundary for the abandoned window.
                    // Only a `WindowSize` can open a window, so it is the only
                    // frame that can prematurely close one.
...
mod test {
    ...
    fn premature_window_size_frame_is_a_fatal_decode_error() {
        ...
        // upstream Logstash/go-lumber server (which reads exactly `window_size`
        // events and treats any other frame type — including a premature `W` —
        // as a fatal error). Rejecting it here means an under-filled window can
        // never reach the acker, so the acker never has to invent a boundary for
        // a window the sender closed early.
        let mut decoder = LogstashDecoder::new();
    ...
    async fn fresh_window_after_completed_window_is_accepted() {
        ...
        // window has completed. Each complete window is ACKed at its boundary.
        let mut decoder = LogstashDecoder::new();
