...
use vector_core::{
    config::{proxy::ProxyConfig, LogNamespace, Output},
    event::Event,
};
...
impl SourceConfig for HttpScrapeConfig {
    async fn build(&self, cx: SourceContext) -> crate::Result<sources::Source> {
        ...
        //     })?;
        let endpoints = vec![self.endpoint];
        //let urls = super::get_urls(&cx, &endpoints)?;
        ...
        let urls = super::get_urls(&endpoints, self.query)?;

        Ok(HttpScrape::http_scrape(
            self,
            &urls,
...
#[async_trait]
impl super::HttpScrape for HttpScrapeConfig {
...
mod test {
    ...
    #[test]
    fn generate_config() {
        crate::test_util::test_generate_config::<HttpScrapeConfig>();
    }
