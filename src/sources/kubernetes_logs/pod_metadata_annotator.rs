...
use lookup::lookup_v2::{parse_value_path_old, OwnedSegment};
...
fn annotate_from_metadata(log: &mut LogEvent, fields_spec: &FieldsSpec, metadata: &ObjectMeta) {
    ...
    if let Some(labels) = &metadata.labels {
        ...
        let prefix_path = parse_value_path_old(&fields_spec.pod_labels);
    ...
    if let Some(annotations) = &metadata.annotations {
        let prefix_path = parse_value_path_old(&fields_spec.pod_annotations);
