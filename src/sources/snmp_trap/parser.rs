    ...
    match message.pdu {
        SnmpPdu::TrapV1(trap) => {
            ...
            if let Some(timestamp_key) = log_schema().timestamp_key() {
                log.insert(
                    (vector_lib::lookup::PathPrefix::Event, timestamp_key),
                    Utc::now(),
                );
            }
            ...
            log.insert(
                "community",
                String::from_utf8_lossy(message.community.as_bytes()).to_string(),
            );
...
fn format_object_value(val: &snmp_parser::snmp::ObjectSyntax) -> String {
    ...
    match val {
        ...
        ObjectSyntax::NsapAddress(addr) => format!("NsapAddress({:?})", addr),
        ObjectSyntax::Empty => "empty".to_string(),
        ObjectSyntax::UnknownSimple(any) => format!("UnknownSimple({:?})", any),
        ObjectSyntax::UnknownApplication(any) => format!("UnknownApplication({:?})", any),
    }
}
...
mod tests {
    ...
    fn test_format_object_value() {
        ...
        // Test basic types that don't require complex ASN.1 construction
        assert_eq!(
        ...
        assert_eq!(format_object_value(&ObjectSyntax::Counter64(400)), "400");
        assert_eq!(format_object_value(&ObjectSyntax::UInteger32(500)), "500");
        assert_eq!(format_object_value(&ObjectSyntax::Empty), "empty");
        assert_eq!(
    ...
    fn test_parse_invalid_data() {
        ...
        let result = parse_snmp_trap(&data, addr, LogNamespace::Legacy);
