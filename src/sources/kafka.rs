                ...
                async move {
                    match message {
                        ...
                        Ok(msg) => {
                            ...
                            log.insert(log_schema().message_key().clone(), Value::from(Bytes::from(payload.to_owned())));
                            ...
                            if let Some(key_field) = &key_field {
                                match msg.key() {
                                    None => (),
                                    Some(key) => {
                                        log.insert(key_field.clone(), Value::from(String::from_utf8_lossy(key).to_string()));
                                    }
                                }
                            }
