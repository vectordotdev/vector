...
impl ConsumerStateInner<Consuming> {
    ...
    fn consume_partition(
        ...
        let handle = join_set.spawn(async move {
            let mut messages = p.stream().ready_chunks(DEFAULT_CHUNK_SIZE_EVENTS);
                        ...
                        Some(msgs) => {
                            ...
                            if let Some(message_handling_tasks_semaphore) = &message_handling_tasks_semaphore {
                                ...
                                processing_futures.push_back(ProcessingFuture::ParsedMessage(tokio::spawn(cancellation_token.run_until_cancelled_owned(async move {
                                    let result = parse_message(msgs, &decoder, decompressor.as_ref(), &keys, acknowledgements, log_namespace).await;
                            ...
                            } else if let Some((events, last, receiver)) = parse_message(msgs, &decoder, decompressor.as_ref(), &keys, acknowledgements, log_namespace).await {
                                let count = events.len();
                                let batch_result = match out.send_batch(events).await {
                                    Err(_) => {
                                        emit!(StreamClosedError { count });
...
async fn parse_message(
    ...
    let streams = messages
        .into_iter()
        .filter_map(|msg| parse_stream(msg, decoder.clone(), decompressor, keys, log_namespace))
    ...
    let payload = match decompressor {
        Some(decompressor) => match decompressor.decompress(&msg.payload) {
            ...
            Err(error) => {
                ...
                return Some(futures::stream::empty().boxed());
            }
        },
