...
mod tests {
    ...
    use crate::{
        ...
        sinks::{
            ...
            splunk_hec::{common::timestamp_key, logs::config::HecLogsEncoding},
    ...
    async fn no_compression_text_event() {
        ...
        let (sink, source) = start(HecLogsEncoding::Text, Compression::None, None).await;
    ...
    async fn one_simple_text_event() {
        ...
        let (sink, source) = start(HecLogsEncoding::Text, Compression::gzip_default(), None).await;
    ...
    async fn one_simple_json_event() {
        ...
        let (sink, source) = start(HecLogsEncoding::Json, Compression::gzip_default(), None).await;
    ...
    async fn event_service_token_passthrough_enabled() {
        assert_source_compliance(&HTTP_PUSH_SOURCE_TAGS, async {
            ...
            let (sink, health) =
                sink(address, HecLogsEncoding::Text, Compression::gzip_default()).await;
            assert!(health.await.is_ok());
...
{"event":"first"}
    ...
    async fn host_test() {
        assert_source_compliance(&HTTP_PUSH_SOURCE_TAGS, async {
            ...
            let (sink, source) =
                start(HecLogsEncoding::Text, Compression::gzip_default(), None).await;
