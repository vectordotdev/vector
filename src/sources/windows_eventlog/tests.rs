...
mod config_tests {
    ...
    fn test_config_validation_empty_channels() {
        ...
        assert!(result.unwrap_err().to_string().contains("At least one channel"));
    }
    ...
    fn test_config_validation_zero_poll_interval() {
        ...
        let result = config.validate();
        assert!(result.is_err());
        assert!(result.unwrap_err().to_string().contains("Poll interval must be greater than 0"));
    }
    ...
    fn test_config_validation_zero_batch_size() {
        ...
        let result = config.validate();
        assert!(result.is_err());
        assert!(result.unwrap_err().to_string().contains("Batch size must be greater than 0"));
    }
    ...
    fn test_config_validation_zero_read_limit() {
        ...
        let result = config.validate();
        assert!(result.is_err());
        assert!(result.unwrap_err().to_string().contains("Read limit must be greater than 0"));
    }
    ...
    fn test_config_validation_empty_channel_name() {
        ...
        let result = config.validate();
        assert!(result.is_err());
        assert!(result.unwrap_err().to_string().contains("Channel names cannot be empty"));
    }
    ...
    fn test_config_validation_empty_query() {
        ...
        let result = config.validate();
        assert!(result.is_err());
        assert!(result.unwrap_err().to_string().contains("Event query cannot be empty"));
    }
    ...
    fn test_event_data_format_configuration() {
        let mut config = create_test_config();
        config.event_data_format.insert("event_id".to_string(), EventDataFormat::String);
        config.event_data_format.insert("process_id".to_string(), EventDataFormat::Integer);
        config.event_data_format.insert("enabled".to_string(), EventDataFormat::Boolean);
...
mod parser_tests {
    ...
    fn test_parse_basic_event() {
        ...
        assert_eq!(log_event.get("level"), Some(&Value::Bytes("Information".into())));
        assert_eq!(log_event.get("level_value"), Some(&Value::Integer(4)));
        assert_eq!(log_event.get("channel"), Some(&Value::Bytes("Security".into())));
        assert_eq!(log_event.get("provider_name"), Some(&Value::Bytes("Microsoft-Windows-Security-Auditing".into())));
        assert_eq!(log_event.get("computer"), Some(&Value::Bytes("WIN-SERVER-01".into())));
    ...
    fn test_parse_event_with_event_data() {
        ...
        if let Some(Value::Object(event_data)) = log_event.get("event_data") {
            assert_eq!(event_data.get("TargetUserName"), Some(&Value::Bytes("admin".into())));
    ...
    fn test_parse_event_with_custom_formatting() {
        let mut config = create_test_config();
        config.event_data_format.insert("event_id".to_string(), EventDataFormat::String);
        config.event_data_format.insert("process_id".to_string(), EventDataFormat::Float);
        ...
        assert_eq!(log_event.get("event_id"), Some(&Value::Bytes("4624".into())));
    ...
    fn test_parse_event_data_xml() {
        ...
        assert_eq!(event_data.get("TargetUserName"), Some(&"administrator".to_string()));
        assert_eq!(event_data.get("TargetLogonId"), Some(&"0x3e7".to_string()));
        assert_eq!(event_data.get("LogonType"), Some(&"2".to_string()));
        assert_eq!(event_data.get("WorkstationName"), Some(&"WIN-TEST".to_string()));
    }
    ...
    fn test_format_value_conversions() {
        ...
        // String conversion
        let result = parser.format_value(&Value::Integer(123), &EventDataFormat::String).unwrap();
        assert_eq!(result, Value::Bytes("123".into()));
        
        // Integer conversion
        let result = parser.format_value(&Value::Bytes("456".into()), &EventDataFormat::Integer).unwrap();
        assert_eq!(result, Value::Integer(456));
        
        // Float conversion
        let result = parser.format_value(&Value::Integer(789), &EventDataFormat::Float).unwrap();
        ...
        let result = parser.format_value(&Value::Bytes("true".into()), &EventDataFormat::Boolean).unwrap();
        assert_eq!(result, Value::Boolean(true));
        
        let result = parser.format_value(&Value::Integer(1), &EventDataFormat::Boolean).unwrap();
        assert_eq!(result, Value::Boolean(true));
        
        // Boolean conversion - falsy values
        let result = parser.format_value(&Value::Bytes("false".into()), &EventDataFormat::Boolean).unwrap();
        assert_eq!(result, Value::Boolean(false));
        
        let result = parser.format_value(&Value::Integer(0), &EventDataFormat::Boolean).unwrap();
        assert_eq!(result, Value::Boolean(false));
        
        // Auto format (no change)
        let original = Value::Integer(999);
        let result = parser.format_value(&original, &EventDataFormat::Auto).unwrap();
    ...
    fn test_format_value_error_handling() {
        ...
        let result = parser.format_value(&Value::Bytes("not_a_number".into()), &EventDataFormat::Integer);
        assert!(result.is_err());
        assert!(result.unwrap_err().to_string().contains("Cannot convert"));
        
        // Invalid float conversion
        let result = parser.format_value(&Value::Bytes("not_a_float".into()), &EventDataFormat::Float);
        assert!(result.is_err());
        assert!(result.unwrap_err().to_string().contains("Cannot convert"));
    }
...
mod error_tests {
    ...
    fn test_error_recoverability() {
        ...
        let recoverable_errors = vec![
            ...
            WindowsEventLogError::ResourceExhaustedError { message: "test".to_string() },
            WindowsEventLogError::IoError { source: std::io::Error::new(std::io::ErrorKind::TimedOut, "timeout") },
        ];
        ...
        let non_recoverable_errors = vec![
            WindowsEventLogError::AccessDeniedError { channel: "Security".to_string() },
            WindowsEventLogError::ChannelNotFoundError { channel: "NonExistent".to_string() },
            WindowsEventLogError::InvalidXPathQuery { query: "invalid".to_string(), message: "syntax error".to_string() },
            WindowsEventLogError::ConfigError { message: "invalid config".to_string() },
        ];
    ...
    fn test_error_user_messages() {
        let error = WindowsEventLogError::AccessDeniedError { channel: "Security".to_string() };
        let message = error.user_message();
        assert!(message.contains("Access denied"));
        assert!(message.contains("Administrator"));
        
        let error = WindowsEventLogError::ChannelNotFoundError { channel: "NonExistent".to_string() };
        let message = error.user_message();
    ...
    fn test_error_conversions() {
        ...
        assert!(matches!(converted, WindowsEventLogError::DatabaseError { .. }));
    }
...
async fn test_source_resources() {
    ...
    assert!(resources.iter().any(|r| r.to_string().contains("Application")));
}
...
async fn test_source_compliance() {
    run_and_assert_source_compliance(&create_test_config(), Duration::from_millis(100), &SOURCE_TAGS).await;
}
