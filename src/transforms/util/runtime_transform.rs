        ...
        let input_rx: MessageStream = Box::new(
            input_rx
                ...
                .map(move |(first, rest)| {
                    ...
                    let init_msg = stream01::once(Ok(Message::Init));
                    // After it comes the first event, if any.
                    let first_event = first.map_or_else(
                        || -> MessageStream { Box::new(stream01::empty()) },
                        |msg| -> MessageStream { Box::new(stream01::once(Ok(msg))) },
                    );
                    // Then all other events followed by `Message::Shutdown` message
                    let rest_events_and_shutdown_msg =
                        rest.chain(stream01::once(Ok(Message::Shutdown)));
                    ...
                    init_msg.chain(first_event).chain(
                        ...
                        rest_events_and_shutdown_msg.weak_select(
                            timer_msgs.chain(stream01::poll_fn(|| Ok(Async::NotReady))),
                        ),
                    )
                })
                .map_err(|_| ())
...
fn make_timer_msgs_stream(timers: Vec<Timer>) -> MessageStream {
    ...
    Box::new(stream::select_all(streams).boxed().compat())
}
