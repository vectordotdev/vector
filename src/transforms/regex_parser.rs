...
use crate::{
    ...
    event::{Event, Value},
    internal_events::{
        RegexParserConversionError, RegexParserMatchError, RegexParserMissingFieldError,
        RegexParserTargetExistsError,
    },
...
impl CompiledRegex {
    ...
    fn captures<'a>(
        ...
        match self.regex.captures_read(&mut self.capture_locs, value) {
                        ...
                        .filter_map(move |(idx, name, conversion)| {
                            capture_locs.get(*idx).and_then(|(start, end)| {
                                ...
                                match conversion.convert(capture) {
                                    ...
                                    Err(error) => {
                                        emit!(&RegexParserConversionError { name, error });
                                        None
                                    }
                                }
                            })
            ...
            None => {
                emit!(&RegexParserMatchError { value });
                None
            }
        }
    }
...
impl FunctionTransform for RegexParser {
    fn transform(&mut self, output: &mut OutputBuffer, mut event: Event) {
        ...
        if let Some(value) = &value {
                ...
                if let Some(target_field) = target_field {
                    if log.contains(target_field) {
                        if self.overwrite_target {
                            ...
                            emit!(&RegexParserTargetExistsError { target_field });
                            output.push(event);
                            return;
                        }
                    }
            ...
            emit!(&RegexParserMissingFieldError { field: &self.field });
        }
