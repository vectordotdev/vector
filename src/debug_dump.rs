...
fn tokio_taskdump_timeout() -> std::time::Duration {
    std::env::var("VECTOR_TOKIO_TASKDUMP_TIMEOUT_SECS")
        .ok()
    ...
    {
        {
            let stderr = std::io::stderr();
            let mut err = stderr.lock();
            let _ = writeln!(err, "--- Tokio task dump ---");
            let _ = err.flush();
        }

        let taskdump_timeout = tokio_taskdump_timeout();
        if let Ok(handle) = tokio::runtime::Handle::try_current() {
            match tokio::time::timeout(taskdump_timeout, handle.dump()).await {
                Ok(dump) => {
                    let stderr = std::io::stderr();
                    let mut err = stderr.lock();
                ...
                Err(_) => {
                    ...
                    let _ = writeln!(
                        err,
                        "[tokio task dump timed out after {}s; a runtime worker may be blocked]",
                        taskdump_timeout.as_secs(),
                    );
                    let _ = err.flush();
                }
            }
            ...
            let stderr = std::io::stderr();
            let mut err = stderr.lock();
            let _ = writeln!(err, "[Tokio runtime handle unavailable for task dump]");
            let _ = err.flush();
        }
    }
