...
impl ClientContext for KafkaStatisticsContext {
    ...
    fn generate_oauth_token(
        ...
        if config.principal_name.is_empty() {
            return Err(
                "OAUTHBEARER principal_name is empty; set sasl.oauthbearer.client.id \
                 or add principal=<name> to sasl.oauthbearer.config"
                    .into(),
            );
        }
...
mod tests {
    ...
    #[test]
    fn generate_token_returns_err_when_principal_name_empty() {
        let ctx = KafkaStatisticsContext {
            expose_lag_metrics: false,
            span: tracing::Span::current(),
            oauthbearer: Some(KafkaOAuthBearerConfig {
