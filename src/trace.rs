...
use metrics_tracing_context::MetricsSubscriber;
...
use tracing::{
    dispatch::{set_global_default, unset_global_default, Dispatch},
    field::{Field, Visit},
    span::Span,
    Id, Metadata,
};
use tracing_core::{collect::Collect, span, Interest};
...
static mut BUFFER: OnceCell<Mutex<Option<Vec<Event>>>> = OnceCell::new();
...
pub fn init(color: bool, json: bool, levels: &str) {
    let _ = unsafe { BUFFER.set(Mutex::new(Some(Vec::new()))) };
    ...
    let dispatch = if json {
        let formatter = tracing_subscriber::fmt::Subscriber::default()
            .json()
            .flatten_event(true);

        let subscriber = subscriber.with(RateLimitedSubscriber::new(formatter));

        if metrics_layer_enabled {
            let subscriber = subscriber.with(MetricsSubscriber::new());
            Dispatch::new(BroadcastSubscriber { subscriber })
        } else {
            Dispatch::new(BroadcastSubscriber { subscriber })
        }
    } else {
        let formatter = tracing_subscriber::fmt::Subscriber::default().with_ansi(color);
...
impl<S: Collect + 'static> Collect for BroadcastSubscriber<S> {
    #[inline]
    ...
    #[inline]
    unsafe fn downcast_raw(&self, id: std::any::TypeId) -> Option<std::ptr::NonNull<()>> {
