...
fn validate_headers(map: &IndexMap<String, String>, auth: &Option<Auth>) -> crate::Result<()> {
    for (name, value) in map {
        ...
        HeaderName::from_bytes(name.as_bytes()).with_context(|_| InvalidHeaderNameSnafu { name })?;
        HeaderValue::from_bytes(value.as_bytes()).with_context(|_| InvalidHeaderValueSnafu { value })?;
    }
