...
pub struct S3Sink<S> {
    acker: Option<Acker>,
    service: Option<S>,
    request_builder: Box<dyn S3RequestBuilder + Send>,
    ...
    fn build_request(&self, key: String, batch: Vec<Event>) -> S3Request {
        {
            ...
            let (body, finalizers) =
                process_event_batch(batch, &mut self.encoding.clone(), self.compression);
...
mod tests {
    ...
    fn s3_encode_event_text() {
        ...
        let _ = encode_event(
            message.clone().into(),
            &mut EncodingConfig::from(Encoding::Text),
            &mut writer,
        )
    ...
    fn s3_encode_event_ndjson() {
        ...
        let _ = encode_event(
            event,
            &mut EncodingConfig::from(Encoding::Ndjson),
            &mut writer,
        )
        .expect("should not have failed to encode event");
        let encoded = writer.into_inner();
    ...
    fn s3_encode_event_with_removed_key() {
        ...
        let mut writer = Cursor::new(Vec::new());
        let _ = encode_event(event, &mut encoding_config, &mut writer)
            .expect("should not have failed to encode event");
        let encoded = writer.into_inner();
        let map: BTreeMap<String, String> = serde_json::from_slice(encoded.as_slice()).unwrap();
