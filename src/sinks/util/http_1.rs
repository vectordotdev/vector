...
use headers_04::HeaderName;
use http_1::{HeaderValue, Request, Response, StatusCode, header};
...
where
    ...
    RL: RetryLogic<Request = <B as Batch>::Output, Response = Response<Bytes>> + Send + 'static,
{
    sink: Arc<T>,
    #[pin]
...
where
    B: Batch,
    B::Output: ByteSizeOf + Clone + Sync + Send + 'static,
    RL: RetryLogic<Request = B::Output, Response = Response<Bytes>, Error = HttpError>
        + Send
        + 'static,
    T: HttpSink<Input = B::Input, Output = B::Output>,
{
...
where
    ...
    T: HttpSink<Input = B::Input, Output = B::Output>,
    RL: RetryLogic<Request = B::Output, Response = Response<Bytes>> + Send + 'static,
{
    sink: Arc<T>,
    #[pin]
...
mod test {
    ...
    use super::*;
    use crate::{config::http_1::ProxyConfig, test_util::addr::next_addr};
    ...
    async fn util_http_it_makes_http_requests() {
        ...
        tokio::spawn(async move {
            ...
            hyper_1::server::conn::http1::Builder::new()
                .serve_connection(
                    io,
                    service_fn(move |req: Request<hyper_1::body::Incoming>| {
                        let mut tx = tx.clone();
                        ...
                        async move {
                            ...
                            Ok::<_, String>(Response::new(Full::new(Bytes::from(""))))
                        }
                    }),
                )
                .await
