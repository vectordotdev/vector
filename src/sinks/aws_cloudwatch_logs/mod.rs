...
fn encode_log(
    ...
    let timestamp = match log.remove(log_schema().timestamp_key(), false) {
...
mod integration_tests {
    ...
    async fn cloudwatch_insert_out_of_range_timestamp() {
        ...
        let mut add_event = |offset: chrono::Duration| {
            ...
            let event = log_event! {
                log_schema().message_key().clone() => line.clone(),
                log_schema().timestamp_key().clone() => now + offset,
            };
            events.push(event);
            line
        };
    ...
    async fn cloudwatch_insert_log_event_partitioned() {
        ...
        let events = input_lines
            ...
            .map(|(i, e)| {
                let event = log_event! {
                    log_schema().message_key().clone() => e,
                    log_schema().timestamp_key().clone() => chrono::Utc::now(),
                    "key" => format!("{}", (i % 2)),
                };
                event
