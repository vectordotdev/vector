...
impl SinkConfig for CloudwatchLogsSinkConfig {
    fn build(&self, cx: SinkContext) -> crate::Result<(super::RouterSink, super::Healthcheck)> {
        ...
        let svc = ServiceBuilder::new()
            .concurrency_limit(request.in_flight_limit.expect("in_flight_limit=auto is not allowed"))
    ...
    for CloudwatchLogsPartitionSvc
{
    ...
    fn call(
        ...
        let svc = if let Some(svc) = &mut self.clients.get_mut(&key) {
            ...
            let svc = ServiceBuilder::new()
                .buffer(self.request_settings.in_flight_limit.expect("in_flight_limit=auto is not allowed"))
                .concurrency_limit(1)
                .rate_limit(
