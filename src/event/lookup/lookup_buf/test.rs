...
lazy_static::lazy_static! {
    static ref SUFFICIENTLY_DECOMPOSED: [SegmentBuf; 9] = [
        SegmentBuf::field(r#"regular"#.to_string()),
        SegmentBuf::field(r#""quoted""#.to_string()),
        SegmentBuf::field(r#""quoted but spaces""#.to_string()),
        SegmentBuf::field(r#""quoted.but.periods""#.to_string()),
        SegmentBuf::field(r#"lookup"#.to_string()),
        SegmentBuf::index(0),
        SegmentBuf::field(r#"nested_lookup"#.to_string()),
        SegmentBuf::index(0),
        SegmentBuf::index(0),
    ];
}
...
fn we_dont_parse_plain_strings_in_from() {
    ...
    assert_eq!(lookup[0], SegmentBuf::field(String::from(input)));
...
fn simple() {
    ...
    assert_eq!(lookup[0], SegmentBuf::field(String::from("some_key")));
    assert_eq!(lookup.to_string(), input);
}
...
fn push() {
    ...
    lookup.push(SegmentBuf::field(String::from(input)));
...
fn pop() {
    ...
    let mut lookup = LookupBuf::from_str(input).unwrap();
    let out = lookup.pop();
    assert_eq!(out, Some(SegmentBuf::field(String::from("some_key"))));
}
...
fn array() {
    ...
    let input = "foo[0]";
    let lookup = LookupBuf::from_str(input).unwrap();
    assert_eq!(lookup[0], SegmentBuf::field(String::from("foo")));
    assert_eq!(lookup[1], SegmentBuf::index(0));
    assert_eq!(lookup.to_string(), input);
}
