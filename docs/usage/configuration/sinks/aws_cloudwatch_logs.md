...
The `aws_cloudwatch_logs` sink batches ["[`log`][log_event]"] up to the `batch_size` or `batch_timeout` options. When flushed, Vector will write to [AWS CloudWatch Logs][aws_cw_logs] via the [`PutLogEvents` API endpoint](https://docs.aws.amazon.com/AmazonCloudWatchLogs/latest/APIReference/API_PutLogEvents.html). The encoding is dictated by the `encoding` option. For example:
