---
description: Batches and flushes log events to [AWS CloudWatch Logs][aws_cw_logs] via the `PutLogEvents` API endpoint
---
...
```coffeescript
[sources.my_aws_cloudwatch_logs_sink]
  # REQUIRED - General
  ...
  batch_timeout = 1049000 # default, bytes
...
| `batch_timeout` | `int` | The maximum age of a batch before it is flushed. See [Batching](#batching) for more info.<br />`default: 1049000` `unit: bytes` |
| **OPTIONAL** - Requests | | |
...
The `aws_cloudwatch_logs` sinks offers an at least once delivery guarantee if your
