---
description: Batches and flushes log events to a [Splunk HTTP Event Collector][splunk_hec]
---
...
The `splunk_hec` sink accepts [`log`][log_event] events.

## Output

The `splunk_hec` sink batches events up to the `batch_size` or `batch_timeout` options. When flushed, Vector will write to a [Splunk HTTP Event Collector][splunk_hec]. The encoding is dictated by the `encoding` option. Below is a templated example:
...
The `splunk_hec` sinks offers an at least once delivery guarantee if your
