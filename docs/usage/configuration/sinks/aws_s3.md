---
description: Batches and flushes log events to [AWS S3][aws_s3] via the `PutObject` API endpoint
---
...
[sources.my_aws_s3_sink]
  ...
  batch_timeout = 10490000 # default, bytes

  # OPTIONAL - Requests
  compression = "gzip" # no default
  encoding = "ndjson" # no default
...
| `batch_timeout` | `int` | The maximum age of a batch before it is flushed. See [Batching](#batching) for more info.<br />`default: 10490000` `unit: bytes` |
| **OPTIONAL** - Requests | | |
...
The `aws_s3` sink accepts [`log`][log_event] events.

## Output

The `aws_s3` sink batches events up to the `batch_size` or `batch_timeout` options. When flushed, Vector will write to [AWS S3][aws_s3] via the [`PutObject` API endpoint](https://docs.aws.amazon.com/AmazonS3/latest/API/RESTObjectPUT.html). The encoding is dictated by the `encoding` option. Below is a templated example:
...
The `aws_s3` sinks offers an at least once delivery guarantee if your
