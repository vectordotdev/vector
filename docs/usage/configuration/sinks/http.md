...
[sources.my_http_sink]
  ...
  # OPTIONAL - Basic auth
  [sources.my_http_sink.basic_auth]
    password = "password" # no default
    user = "username" # no default

  # OPTIONAL - Headers
  [sources.my_http_sink.headers]
...
The `http` sink accepts [`log`][log_event] events.

## Output

The `http` sink batches events up to the `batch_size` or `batch_timeout` options. When flushed, Vector will write to a generic HTTP endpoint. The encoding is dictated by the `encoding` option. Below is a templated example:
...
The `http` sinks offers an at least once delivery guarantee if your
