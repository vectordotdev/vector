---
description: Accept log events through one or more local files
---
...
The `file` source allows you to ingest [`log`][log_event] through one or more local files.
...
The `file` source outputs [`log`][log_event] events with the following schema:

| Key  | Type  | Description |
| :--- | :---: | :---------- |
| `file` | `string` | The source log file.You can customize the name via the `file_key` config option.<br />`example: "/var/log/nginx.log"` |
...
Due to nature of ingesting data through one or more local files, the `file` source
