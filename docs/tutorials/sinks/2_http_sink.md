...
is split out into metadata and actual event data. This event data is encoded
using the encoder we created earlier. The results from the encoding are passed
along with the metadata to create the final request that is passed to the
`Tower` sink.
