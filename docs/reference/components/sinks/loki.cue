...
components: sinks: loki: {
	...
	configuration: {
		...
		labels: {
			description: "A set of labels that will be attached to each batch of events. Both keys and values are templateable to allow events to provide dynamic labels. Note: If the set of labels has high cardinality this can cause drastic performance issues with Loki. To ensure this does not happen one should try to reduce the amount of unique label keys and values."
			required:    true
			warnings: []
			type: object: {
				examples: [
