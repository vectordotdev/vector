		...
		fields: {
			...
			prometheusSink: {
				description: """
					...
					When using this 'built-in' sink, we automatically configure container ports, and ensure things are
					ready for discovery and scraping via Prometheus' `kubernetes_sd_configs` jobs.
					"""
				...
				type: object: options: {
					...
					vectorSink: {
						description: """
							The 'built-in' vector sink, to send the logs to the vector aggregator. Disabled by default
							when using vector-agent chart. Enabled by default when using `vector` chart, and
