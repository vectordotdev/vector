...
remap: functions: push: {
	...
	category: "Array"
	description: """
		Adds the specified item to a the front of an array and returns the resulting array. The item
		can be of any VRL type and is added even if an item with the same value is already present
		in the array.

		The `push` function does *not* change the object in place. This, for example, would not
		return an array with `apple`, `orange`, and `banana`, but the value of `fruits` would be
		unchanged:
