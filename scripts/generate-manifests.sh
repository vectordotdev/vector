...
for values in "$TMPDIR"/values-*.yaml; do
    ...
    mkdir -p "$dest"
    # Remove manifests the chart no longer emits; keep hand-maintained files
    # (e.g. kustomization.yaml) that this script does not generate.
    find "$dest" -maxdepth 1 -type f ! -name kustomization.yaml -delete
