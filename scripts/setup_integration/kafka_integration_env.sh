...
ZOOKEEPER=docker.io/wurstmeister/zookeeper
KAFKA=docker.io/wurstmeister/kafka:2.13-2.6.0
...
start_podman () {
  ...
  podman run -d --pod=vector-test-integration-kafka -e KAFKA_BROKER_ID=1 \
	 ...
	 -v "$(pwd)"/tests/data/kafka_server_jaas.conf:/etc/kafka/kafka_server_jaas.conf --name vector_kafka $KAFKA
}

start_docker () {
  docker network create vector-test-integration-kafka
  docker run -d --network=vector-test-integration-kafka -p 2181:2181 --name vector_zookeeper $ZOOKEEPER
   # pinned to 2.6.0 due to https://github.com/wurstmeister/kafka-docker/issues/669
